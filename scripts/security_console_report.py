...
COL = (
    ...
    if supports_color()
    else {k: "" for k in ["bold", "red", "yellow", "green", "cyan", "reset"]}
)
...
def main():
    ...
    if lambdas_dir.exists():
        for lam in lambdas_dir.iterdir():
            ...
            if not pd:
                dep_vulns_by_lambda[lam.name] = {
                    "error": "pip-audit not available or failed",
                    "vulnerabilities": [],
                }
                continue
                    ...
                    vulns.append(
                        {
                            ...
                            "version": version,
                            "id": v.get("id"),
                            "fix": v.get("fix_version") or v.get("fixed_in"),
                            "severity": sev,
            ...
            if parse_error and not vulns:
                ...
                dep_vulns_by_lambda[lam.name] = {
                    "error": "unexpected pip-audit output format",
                    "raw": pd,
                    "vulnerabilities": [],
                }
                continue
