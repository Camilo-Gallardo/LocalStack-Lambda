...
class ResponseParser:
    ...
    def _is_modeled_error_shape(
        ...
        return shape is not None and shape.metadata.get(
            "exception",
            False,
        )
...
class BaseXMLResponseParser(ResponseParser):
    ...
    def _handle_map(
        ...
        if shape.serialization.get("flattened") and not isinstance(
            node,
            list,
        ):
            node = [node]
...
class BaseJSONParser(ResponseParser):
    ...
    def _do_error_parse(
        ...
        code = body.get(
            "__type",
            response_code and str(response_code),
        )
        if code is not None:
...
class BaseRestParser(ResponseParser):
    ...
    def _handle_list(
        ...
        if location == "header" and not isinstance(
            node,
            list,
        ):
