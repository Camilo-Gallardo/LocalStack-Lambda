...
def handler(event, context):
    ...
    and return a list of missions with their videos and corresponding transcripts.
    """
    ...
    for mission_folder in mission_folders:
        ...
        # Find videos and transcripts subfolders within this mission
        videos_folder_id = get_subfolder_by_name(access_token, mission_id, "videos")
        transcripts_folder_id = get_subfolder_by_name(
            access_token, mission_id, "transcripts"
        )

        if not videos_folder_id:
            logger.warning(f"No 'videos' folder found in mission: {mission_name}")
            continue

        if not transcripts_folder_id:
            logger.warning(f"No 'transcripts' folder found in mission: {mission_name}")

        # Get files from videos folder
        video_files = list_sharepoint_files(access_token, videos_folder_id)
        if video_files is None:
            logger.error(f"Failed to retrieve video files for mission: {mission_name}")
            continue

        # Get files from transcripts folder (if it exists)
        transcript_files = []
        if transcripts_folder_id:
            transcript_files = list_sharepoint_files(
                access_token, transcripts_folder_id
            )
        ...
        # Create lookup for transcripts by normalized name
        transcripts_by_name = {
        ...
        for video in video_files:
            ...
            matching_transcript = transcripts_by_name.get(video_name_normalized)
