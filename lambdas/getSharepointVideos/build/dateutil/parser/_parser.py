...
class _timelex(object):
    ...
    def get_token(
        ...
        if state in (
            "a.",
            "0.",
        ) and (seenletters or token.count(".") > 1 or token[-1] in ".,"):
...
class parserinfo(object):
    ...
    def validate(
        ...
        if res.year is not None:
            res.year = self.convertyear(
                res.year,
                res.century_specified,
            )
...
class parser(object):
    ...
    def _parse(
        ...
        if fuzzy_with_tokens:
            skipped_tokens = self._recombine_skipped(
                l,
                skipped_idxs,
            )
            return (
                res,
                tuple(skipped_tokens),
            )
        else:
            return (
                res,
    ...
    def _parse_numeric_token(
        ...
        try:
            ...
            six.raise_from(
                ValueError("Unknown numeric token"),
                e,
            )
        ...
        if (
            ...
            (
                res.minute,
                res.second,
            ) = self._parse_min_sec(value)

            if idx + 4 < len_l and tokens[idx + 3] == ":":
                (
                    res.second,
                    res.microsecond,
                ) = self._parsems(tokens[idx + 4])
    ...
    def _build_tzaware(
        ...
        if callable(tzinfos) or (tzinfos and res.tzname in tzinfos):
            tzinfo = self._build_tzinfo(
                tzinfos,
                res.tzname,
                res.tzoffset,
            )
            aware = naive.replace(tzinfo=tzinfo)
            aware = self._assign_tzname(
                aware,
                res.tzname,
            )
    ...
    def _recombine_skipped(
        ...
        for (
            i,
            idx,
        ) in enumerate(sorted(skipped_idxs)):
