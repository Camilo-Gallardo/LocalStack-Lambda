...
class relativedelta(object):
    ...
    def normalized(
        ...
        hours_f = round(
            self.hours + 24 * (self.days - days),
            11,
        )
        hours = int(hours_f)

        minutes_f = round(
            self.minutes + 60 * (hours_f - hours),
            10,
        )
        minutes = int(minutes_f)

        seconds_f = round(
            self.seconds + 60 * (minutes_f - minutes),
            8,
        )
    ...
    def __add__(
        ...
        elif self._has_time and not isinstance(
            other,
            datetime.datetime,
        ):
