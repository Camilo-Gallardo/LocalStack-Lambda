...
try:
    ...
    _zstd_version = _zstd_version = tuple(map(int, re.search(r"^([0-9]+)\.([0-9]+)", zstd.__version__).groups()))  # type: ignore[union-attr]
    if (
        _zstd_version
        < (
            0,
...
class DeflateDecoder(
    ...
    def decompress(
        ...
        self._data += data
        try:
            decompressed = self._obj.decompress(
                data
            )
            if decompressed:
    ...
    def decompress(
        ...
        while True:
            try:
                ...
                if (
                    previous_state
                    == GzipDecoderState.OTHER_MEMBERS
                ):
...
class BaseHTTPResponse(
    ...
    def _init_decoder(
        ...
        if (
            ...
            if (
                content_encoding
                in self.CONTENT_DECODERS
            ):
                self._decoder = _get_decoder(
                    content_encoding
                )
            elif (
                ","
...
class HTTPResponse(
    ...
    def read(
        ...
        if (
            ...
            if (
                ...
                return self._decoded_buffer.get(
                    amt
                )
    ...
    def stream(
        ...
        ** 16,
        ...
        if (
            self.chunked
            and self.supports_chunked_reads()
        ):
    ...
    def read_chunked(
        ...
        if (
            not self.supports_chunked_reads()
        ):
