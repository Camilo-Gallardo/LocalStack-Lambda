...
class PoolManager(
    ...
    def connection_from_host(
        ...
        return self.connection_from_context(
            request_context
        )
    ...
    def connection_from_context(
        ...
        scheme = request_context[
            "scheme"
        ].lower()
        pool_key_constructor = self.key_fn_by_scheme.get(
            scheme
        )
        if (
            not pool_key_constructor
        ):
            raise URLSchemeUnknown(
                scheme
            )
        pool_key = pool_key_constructor(
            request_context
        )
    ...
    def urlopen(self, method: str, url: str, redirect: bool = True, **kw: typing.Any) -> BaseHTTPResponse:  # type: ignore[override]
        """
        ...
        if self._proxy_requires_url_absolute_form(
            u
        ):
        ...
        redirect_location = (
            redirect
            and response.get_redirect_location()
        )
        if (
...
class ProxyManager(
    ...
    def connection_from_host(
        ...
        return super().connection_from_host(self.proxy.host, self.proxy.port, self.proxy.scheme, pool_kwargs=pool_kwargs)  # type: ignore[union-attr]
