...
_OP_NO_SSLv2_OR_SSLv3: int = getattr(
    OpenSSL.SSL,
    "OP_NO_SSLv2",
    0,
) | getattr(
...
_OP_NO_TLSv1: int = getattr(
    OpenSSL.SSL,
    "OP_NO_TLSv1",
    0,
)
_OP_NO_TLSv1_1: int = getattr(
    OpenSSL.SSL,
    "OP_NO_TLSv1_1",
    0,
)
_OP_NO_TLSv1_2: int = getattr(
    OpenSSL.SSL,
    "OP_NO_TLSv1_2",
    0,
)
_OP_NO_TLSv1_3: int = getattr(
    OpenSSL.SSL,
    "OP_NO_TLSv1_3",
    0,
)

_openssl_to_ssl_minimum_version: dict[
    int,
    int,
] = {
...
_openssl_to_ssl_maximum_version: dict[
    int,
    int,
] = {
...
def get_subj_alt_name(
    ...
    names.extend(
        ...
        for name in ext.get_values_for_type(
            x509.IPAddress
        )
    )
...
class WrappedSocket:
    ...
    def getpeercert(
        ...
        return {
            ...
            "subjectAltName": get_subj_alt_name(
                x509
            ),
        }
...
class PyOpenSSLContext:
    ...
    def __init__(
        ...
        self.protocol = _openssl_versions[
            protocol
        ]
        self._ctx = OpenSSL.SSL.Context(
            self.protocol
        )
        self._options = 0
    ...
    def verify_mode(
        ...
        return _openssl_to_stdlib_verify[
            self._ctx.get_verify_mode()
        ]

    @verify_mode.setter
    def verify_mode(
        self,
        value: ssl.VerifyMode,
    ) -> None:
    ...
    def set_ciphers(
        ...
        if isinstance(
            ...
            ciphers = ciphers.encode(
                "utf-8"
            )
        self._ctx.set_cipher_list(
            ciphers
        )
    ...
    def load_cert_chain(
        ...
        try:
            self._ctx.use_certificate_chain_file(
                certfile
            )
            if (
                password
    ...
    def wrap_socket(
        ...
        if (
            server_hostname
            and not util.ssl_.is_ipaddress(
                server_hostname
            )
        ):
            if isinstance(
