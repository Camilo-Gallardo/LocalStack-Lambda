...
_IPV6_ADDRZ_RE = re.compile(
    "^"
    + _IPV6_ADDRZ_PAT
    + "$"
)
_BRACELESS_IPV6_ADDRZ_RE = re.compile(
    "^"
    + _IPV6_ADDRZ_PAT[
        2:-2
    ]
...
def _encode_invalid_chars(
    component: str,
    allowed_chars: typing.Container[
        str
    ],
) -> str:  # Abstract
    ...
...
def _normalize_host(
    ...
    if host:
        if (
            scheme
            in _NORMALIZABLE_SCHEMES
        ):
            is_ipv6 = _IPV6_ADDRZ_RE.match(
                host
            )
            if is_ipv6:
...
def _encode_target(
    ...
    if (
        ...
        raise LocationParseError(
            f"{target!r} is not a valid request URI"
        )
