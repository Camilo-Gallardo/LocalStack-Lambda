...
_TYPE_VERSION_INFO = typing.Tuple[
    int,
    int,
    int,
    str,
...
if (
    ...
    from .ssltransport import (
        SSLTransport as SSLTransportType,
    )

    class _TYPE_PEER_CERT_RET_DICT(
        TypedDict,
        total=False,
    ):
...
_SSL_VERSION_TO_TLS_VERSION: dict[
    int,
    int,
] = (
    {}
...
def assert_fingerprint(
    ...
    fingerprint_bytes = unhexlify(
        fingerprint.encode()
    )

    cert_digest = hashfunc(
        cert
    ).digest()
...
def is_ipaddress(
    ...
    return bool(
        ...
        or _BRACELESS_IPV6_ADDRZ_RE.match(
            hostname
        )
    )
