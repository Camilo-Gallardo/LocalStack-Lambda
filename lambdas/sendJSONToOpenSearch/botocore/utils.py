...
def resolve_imds_endpoint_mode(
    session,
):
    ...
    if (
        ...
        if (
            lendpoint_mode
            not in METADATA_ENDPOINT_MODES
        ):
            error_msg_kwargs = {
                "mode": endpoint_mode,
                "valid_modes": METADATA_ENDPOINT_MODES,
            }
            raise InvalidIMDSEndpointModeError(
                **error_msg_kwargs
            )
        return lendpoint_mode
    elif session.get_config_variable(
        "imds_use_ipv6"
    ):
        return "ipv6"
    return "ipv4"
...
def has_header(
    ...
    if (
        ...
        headers,
        botocore.awsrequest.HeadersDict,
    ):
        return (
            header_name
            in headers
...
class IMDSFetcher:
    ...
    def __init__(
        ...
        self._base_url = self._select_base_url(
            base_url,
            config,
        )
        self._config = config
    ...
    def _select_base_url(
        ...
        custom_metadata_endpoint = config.get(
            "ec2_metadata_service_endpoint"
        )

        if (
            requires_ipv6
            and custom_metadata_endpoint
        ):
            logger.warning(
    ...
    def _construct_url(
        ...
        if (
            self._base_url
            and not self._base_url.endswith(
                "/"
            )
        ):
            sep = "/"
    ...
    def _default_retry(
        ...
        return self._is_non_ok_response(
            response
        ) or self._is_empty(
            response
        )

    def _is_non_ok_response(
        self,
        response,
    ):
        if (
...
class InstanceMetadataFetcher(
    ...
    def retrieve_iam_role_credentials(
        self,
    ):
        try:
            token = (
                self._fetch_metadata_token()
            )
            ...
            if self._contains_all_credential_fields(
                credentials
            ):
                credentials = {
                    "role_name": role_name,
    ...
    def _needs_retry_for_role_name(
        self,
        response,
    ):
        return self._is_non_ok_response(
    ...
    def _needs_retry_for_credentials(
        self,
        response,
    ):
        return (
            self._is_non_ok_response(
                response
            )
            or self._is_empty(
                response
            )
            or self._is_invalid_json(
                response
            )
        )

    def _contains_all_credential_fields(
        self,
        credentials,
    ):
        for field in (
            self._REQUIRED_CREDENTIAL_FIELDS
        ):
            if (
                field
                not in credentials
...
class IMDSRegionProvider:
    ...
    def provide(
        ...
        instance_region = (
            self._get_instance_metadata_region()
        )
        return instance_region

    def _get_instance_metadata_region(
        self,
    ):
        fetcher = (
            self._get_fetcher()
...
def parse_key_val_file(
    ...
    try:
        with _open(
            ...
            return parse_key_val_file_contents(
                contents
            )
    except OSError:
...
def parse_key_val_file_contents(
    contents,
):
...
def _epoch_seconds_to_datetime(
    value,
    tzinfo,
):
    ...
    epoch_zero_localized = epoch_zero.astimezone(
        tzinfo()
    )
    return (
        epoch_zero_localized
        + datetime.timedelta(
            seconds=value
        )
    )
...
def _parse_timestamp_with_tzinfo(
    value,
    tzinfo,
):
...
def parse_timestamp(
    ...
    for tzinfo in tzinfo_options:
        try:
            return _parse_timestamp_with_tzinfo(
                value,
                tzinfo,
            )
        except (
    ...
    try:
        ...
        try:
            for tzinfo in tzinfo_options:
                return _epoch_seconds_to_datetime(
                    numeric_value,
                    tzinfo=tzinfo,
                )
        except (
    ...
    raise RuntimeError(
        'Unable to calculate correct timezone offset for "%s"'
        % value
    )
    ...
    def _generate_skeleton(
        ...
        try:
            if (
                ...
                return self._generate_type_structure(
                    shape,
                    stack,
                )
            elif (
                ...
                return self._generate_type_list(
                    shape,
                    stack,
                )
            elif (
                ...
                return self._generate_type_map(
                    shape,
                    stack,
                )
            elif (
...
def is_valid_ipv6_endpoint_url(
    endpoint_url,
):
    if UNSAFE_URL_CHARS.intersection(
        endpoint_url
    ):
        return False
...
def is_valid_ipv4_endpoint_url(
    endpoint_url,
):
    hostname = urlparse(
        endpoint_url
...
def is_valid_uri(
    ...
    return is_valid_endpoint_url(
        endpoint_url
    ) or is_valid_ipv6_endpoint_url(
        endpoint_url
    )
...
def check_dns_name(
    ...
    if match is None or match.end() != len(
        bucket_name
    ):
        return False
    return True
...
def fix_s3_host(
    ...
    **kwargs,
    ...
    try:
        ...
        bucket_name = e.kwargs[
            "bucket_name"
        ]
        logger.debug(
            "Not changing URI, bucket is not DNS compatible: %s",
            bucket_name,
        )
...
def switch_to_virtual_host_style(
    ...
    **kwargs,
    ...
    if (
        ...
        # customization again.
        return
    elif _is_get_bucket_location_request(
        request
    ):
    ...
    if (
        ...
        if (
            ...
            return
        logger.debug(
            "Checking for DNS compatible bucket for: %s",
            request.url,
        )
        if check_dns_name(
            bucket_name
...
def _is_get_bucket_location_request(
    request,
):
    ...
    **This is not a general purpose caching decorator.**
    ...
    def _cache_guard(
        ...
        **kwargs,
        ...
        result = self._instance_cache.get(
            cache_key
        )
        if (
            result
...
def _get_new_endpoint(
    original_endpoint,
    new_endpoint,
    use_new_scheme=True,
):
    new_endpoint_components = urlsplit(
        new_endpoint
    )
    original_endpoint_components = urlsplit(
        original_endpoint
    )
    scheme = (
        original_endpoint_components.scheme
    )
    if use_new_scheme:
        scheme = (
    ...
    final_endpoint = urlunsplit(
        final_endpoint_components
    )
    logger.debug(
        f"Updating URI from {original_endpoint} to {final_endpoint}"
    )
...
class S3RegionRedirectorv2:
    ...
    def register(
        ...
        emitter = (
            event_emitter
            or self._client.meta.events
        )
        emitter.register(
            "needs-retry.s3",
            self.redirect_from_error,
        )
        emitter.register(
            "before-parameter-build.s3",
            self.annotate_request_context,
        )
        emitter.register(
    ...
    def redirect_from_error(
        ...
        **kwargs,
        ...
        client_region = request_dict[
            "context"
        ].get(
            "client_region"
        )
        new_region = self.get_bucket_region(
            bucket,
            response,
        )
        ...
        auth_schemes = ep_info.properties.get(
            "authSchemes"
        )
        if (
            auth_schemes
            ...
            auth_info = ep_resolver.auth_schemes_to_signing_ctx(
                auth_schemes
            )
            (
                auth_type,
    ...
    def get_bucket_region(
        ...
        response_headers = service_response[
            "ResponseMetadata"
        ][
            "HTTPHeaders"
        ]
        ...
        try:
            response = self._client.head_bucket(
                Bucket=bucket
            )
            headers = response[
                "ResponseMetadata"
...
class S3RegionRedirector:
    ...
    def redirect_from_cache(
        ...
        **kwargs,
        ...
        signing_context = self._cache.get(
            bucket
        )
        if (
            signing_context
...
class S3ArnParamHandler:
    ...
    def handle_arn(
        ...
        **kwargs,
        ...
        if (
            model.name
            in self._BLACKLISTED_OPERATIONS
        ):
            return
        arn_details = self._get_arn_details_from_bucket_param(
            params
        )
        if (
            arn_details
    ...
    def _get_arn_details_from_bucket_param(
        self,
        params,
    ):
        if (
            ...
            try:
                ...
                arn_details = self._arn_parser.parse_arn(
                    arn
                )
                self._add_resource_type_and_name(
                    arn,
                    arn_details,
                )
                return arn_details
    ...
    def _add_resource_type_and_name(
        self,
        arn,
        arn_details,
    ):
        match = self._RESOURCE_REGEX.match(
            arn_details[
                "resource"
            ]
        )
        if match:
            ...
            raise UnsupportedS3ArnError(
                arn=arn
            )

    def _store_accesspoint(
        self,
        params,
        context,
        arn_details,
    ...
    def _store_outpost(
        ...
        resource_name = arn_details[
            "resource_name"
        ]
        match = self._OUTPOST_RESOURCE_REGEX.match(
            resource_name
        )
        if (
            not match
        ):
            raise UnsupportedOutpostResourceError(
...
class S3EndpointSetter:
    ...
    def set_endpoint(
        ...
        **kwargs,
        ...
        if self._use_accesspoint_endpoint(
            request
        ):
            self._validate_accesspoint_supported(
                request
            )
            self._validate_fips_supported(
                request
            )
            self._validate_global_regions(
                request
            )
            ...
            self._switch_to_accesspoint_endpoint(
                request,
                region_name,
            )
            return
    ...
    def _validate_fips_supported(
        self,
        request,
    ):
        if (
            not self._use_fips_endpoint
        ):
            return
        if (
            "fips"
        ...
        accesspoint_region = request.context[
            "s3_accesspoint"
        ][
            "region"
        ]
    ...
    def _validate_accesspoint_supported(
        self,
        request,
    ):
        if (
        ...
        request_partition = request.context[
            "s3_accesspoint"
        ][
            "partition"
        ]
        ...
        self._validate_mrap_s3_config(
            request
        )

    def _validate_mrap_s3_config(
        self,
        request,
    ):
        if not is_global_accesspoint(
            request.context
        ):
            return
        if self._s3_config.get(
    ...
    def _resolve_region_for_accesspoint_endpoint(
        self,
        request,
    ):
        if is_global_accesspoint(
            request.context
        ):
            # Requests going to MRAP endpoints MUST be set to any (*) region.
            self._override_signing_region(
                request,
                "*",
            )
        elif self._s3_config.get(
    ...
    def _resolve_signing_name_for_accesspoint_endpoint(
        self,
        request,
    ):
        accesspoint_service = request.context[
            "s3_accesspoint"
        ][
            "service"
        ]
    ...
    def _switch_to_accesspoint_endpoint(
        self,
        request,
        region_name,
    ):
        original_components = urlsplit(
            request.url
        )
    ...
    def _get_netloc(
        ...
        if is_global_accesspoint(
            ...
            return self._get_mrap_netloc(
                request_context
            )
        else:
            return self._get_accesspoint_netloc(
                request_context,
                region_name,
            )

    def _get_mrap_netloc(
        self,
        request_context,
    ):
        s3_accesspoint = request_context[
            "s3_accesspoint"
        ]
        ...
        if (
            ...
            endpoint_url_netloc = urlsplit(
                self._endpoint_url
            ).netloc
            mrap_netloc_components.append(
                endpoint_url_netloc
            )
        else:
            partition = s3_accesspoint[
                "partition"
            ]
            mrap_netloc_components.extend(
                [
                    "accesspoint",
                    region_name,
                    self._get_partition_dns_suffix(
                        partition
                    ),
                ]
            )
    ...
    def _get_accesspoint_netloc(
        ...
        outpost_name = s3_accesspoint.get(
            "outpost_name"
        )
        if (
            self._endpoint_url
            ...
            if outpost_name:
                accesspoint_netloc_components.append(
                    outpost_name
                )
            endpoint_url_netloc = urlsplit(
                self._endpoint_url
    ...
    def _override_signing_region(
        self,
        request,
        region_name,
    ):
        signing_context = request.context.get(
            "signing",
            {},
        )
    ...
    def _override_signing_name(
        self,
        context,
        signing_name,
    ):
        signing_context = context.get(
            "signing",
            {},
        )
    ...
    def _addressing_style(
        ...
        configured_addressing_style = self._s3_config.get(
            "addressing_style"
        )
    ...
    def _s3_addressing_handler(
        ...
        if (
            ...
            logger.debug(
                "Using S3 virtual host style addressing."
            )
...
class S3ControlEndpointSetter:
    ...
    def set_endpoint(
        ...
        **kwargs,
        ...
        if self._use_endpoint_from_arn_details(
            request
        ):
            self._validate_endpoint_from_arn_details_supported(
                request
            )
            region_name = self._resolve_region_from_arn_details(
                request
            )
            self._resolve_signing_name_from_arn_details(
                request
            )
            self._resolve_endpoint_from_arn_details(
                request,
                region_name,
            )
            self._add_headers_from_arn_details(
                request
            )
        elif self._use_endpoint_from_outpost_id(
            request
        ):
            self._validate_outpost_redirection_valid(
                request
            )
            self._override_signing_name(
                request,
            ...
            new_netloc = self._construct_outpost_endpoint(
                self._region
            )
            self._update_request_netloc(
                request,
                new_netloc,
            )

    def _use_endpoint_from_arn_details(
        self,
        request,
    ):
        return (
    ...
    def _use_endpoint_from_outpost_id(
        self,
        request,
    ):
        return (
    ...
    def _validate_endpoint_from_arn_details_supported(
        self,
        request,
    ):
        if (
        ...
        request_partion = request.context[
            "arn_details"
        ][
            "partition"
        ]
    ...
    def _validate_outpost_redirection_valid(
        self,
        request,
    ):
        if self._s3_config.get(
    ...
    def _resolve_region_from_arn_details(
        self,
        request,
    ):
        if self._s3_config.get(
    ...
    def _resolve_signing_name_from_arn_details(
        self,
        request,
    ):
        arn_service = request.context[
            "arn_details"
        ][
            "service"
        ]
    ...
    def _resolve_endpoint_from_arn_details(
        self,
        request,
        region_name,
    ):
    ...
    def _resolve_netloc_from_arn_details(
        ...
        if (
            ...
            return self._construct_outpost_endpoint(
                region_name
            )
        account = arn_details[
            "account"
        ]
        return self._construct_s3_control_endpoint(
            region_name,
            account,
        )

    def _is_valid_host_label(
        self,
        label,
    ):
        return self._HOST_LABEL_REGEX.match(
            label
        )

    def _validate_host_labels(
        self,
        *labels,
    ):
        for label in labels:
            if not self._is_valid_host_label(
                label
            ):
                raise InvalidHostLabelError(
                    label=label
                )

    def _construct_s3_control_endpoint(
        self,
        region_name,
        account,
    ):
        self._validate_host_labels(
            region_name,
            account,
        )
        if (
        ...
        return self._construct_netloc(
            netloc
        )

    def _construct_outpost_endpoint(
        self,
        region_name,
    ):
        self._validate_host_labels(
    ...
    def _add_headers_from_arn_details(
        self,
        request,
    ):
        arn_details = request.context[
        ...
        outpost_name = arn_details.get(
            "outpost_name"
        )
        if outpost_name:
            self._add_outpost_id_header(
                request,
                outpost_name,
            )

    def _add_outpost_id_header(
        self,
        request,
        outpost_name,
    ):
...
class S3ControlArnParamHandler:
    ...
    _RESOURCE_SPLIT_REGEX = re.compile(
        r"[/:]"
    )
    ...
    def handle_arn(
        ...
        **kwargs,
        ...
        if (
            ...
            self._handle_outpost_id_param(
                params,
                model,
                context,
            )
        else:
            self._handle_name_param(
                params,
                model,
                context,
            )
            self._handle_bucket_param(
                params,
                model,
                context,
            )

    def _get_arn_details_from_param(
        self,
        params,
        param_name,
    ):
    ...
    def _split_resource(
        ...
        return self._RESOURCE_SPLIT_REGEX.split(
            arn_details[
                "resource"
            ]
        )

    def _override_account_id_param(
        self,
        params,
        arn_details,
    ):
        account_id = arn_details[
            "account"
        ]
        if (
            "AccountId"
    ...
    def _handle_name_param(
        ...
        if (
            ...
            return
        arn_details = self._get_arn_details_from_param(
            params,
            "Name",
        )
        if (
            ...
            return
        if self._is_outpost_accesspoint(
            arn_details
        ):
            self._store_outpost_accesspoint(
                params,
                context,
                arn_details,
            )
    ...
    def _store_outpost_accesspoint(
        self,
        params,
        context,
        arn_details,
    ):
        self._override_account_id_param(
            params,
            arn_details,
        )
        accesspoint_name = arn_details[
            "resources"
        ][
            3
        ]
    ...
    def _handle_bucket_param(
        ...
        if (
            ...
            return
        if self._is_outpost_bucket(
            arn_details
        ):
            self._store_outpost_bucket(
                params,
                context,
                arn_details,
            )
...
class S3ControlArnParamHandlerv2(
    ...
    def _handle_name_param(
        ...
        if (
            ...
            return
        self._raise_for_fips_pseudo_region(
            arn_details
        )
        self._raise_for_accelerate_endpoint(
            context
        )
        if self._is_outpost_accesspoint(
            arn_details
    ...
    def _raise_for_fips_pseudo_region(
        self,
        arn_details,
    ):
        # FIPS pseudo region names cannot be used in ARNs
        arn_region = arn_details[
            "region"
        ]
        if arn_region.startswith(
            "fips-"
        ) or arn_region.endswith(
            "fips-"
        ):
            raise UnsupportedS3ControlArnError(
                arn=arn_details[
    ...
    def _raise_for_accelerate_endpoint(
        self,
        context,
    ):
        s3_config = (
...
class ContainerMetadataFetcher:
    ...
    def retrieve_full_uri(
        ...
        """
        self._validate_allowed_url(
            full_url
        )
        return self._retrieve_credentials(
            full_url,
            headers,
        )
    ...
    def _validate_allowed_url(
        ...
        parsed = botocore.compat.urlparse(
            full_url
        )
    ...
    def _check_if_whitelisted_host(
        self,
        host,
    ):
        if (
            host
            in self._ALLOWED_HOSTS
        ):
            return True
        return False
...
def calculate_md5(
    ...
    **kwargs,
    ...
    if isinstance(
        ...
        binary_md5 = _calculate_md5_from_bytes(
            body
        )
    else:
        binary_md5 = _calculate_md5_from_file(
            body
        )
...
def conditionally_calculate_md5(
    params,
    **kwargs,
):
    ...
    checksum_algorithm = checksum_context.get(
        "request_algorithm"
    )
    if (
        checksum_algorithm
    ...
    for header in headers:
        if CHECKSUM_HEADER_PATTERN.match(
            header
        ):
            return
    if (
...
class SSOTokenLoader:
    ...
    def __call__(
        ...
        logger.debug(
            f"Checking for cached token at: {cache_key}"
        )
        if (
            cache_key
            not in self._cache
...
class EventbridgeSignerSetter:
    ...
    def _get_global_endpoint(
        ...
        partition = resolver.get_partition_for_region(
            self._region
        )
        if (
            partition
...
class JSONFileCache:
    ...
    def _default_dumps(
        ...
        return json.dumps(
            obj,
            default=self._serialize_if_needed,
        )
    ...
    def __contains__(
        ...
        actual_key = self._convert_cache_key(
            cache_key
        )
    ...
    def __setitem__(
        ...
        full_key = self._convert_cache_key(
            cache_key
        )
        try:
            file_content = self._dumps(
                value
            )
        except (
            TypeError,
