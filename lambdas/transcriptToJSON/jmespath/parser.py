...
class Parser(
    ...
    def _do_parse(
        ...
        try:
            ...
            exceptions.LexerError
        ) as e:
            e.expression = expression
            raise
        except (
            exceptions.IncompleteExpressionError
        ) as e:
            e.set_expression(
                expression
            )
    ...
    def _parse(
        ...
        self.tokenizer = lexer.Lexer().tokenize(
            expression
        )
        self._tokens = list(
            self.tokenizer
    ...
    def _expression(
        ...
        left_token = self._lookahead_token(
            0
        )
        self._advance()
        ...
        while (
            ...
            if (
                ...
                error_token = self._lookahead_token(
                    0
                )
                self._error_led_token(
                    error_token
                )
            else:
                self._advance()
    ...
    def _token_nud_unquoted_identifier(
        self,
        token,
    ):
        return ast.field(
    ...
    def _token_nud_quoted_identifier(
        self,
        token,
    ):
        field = ast.field(
    ...
    def _token_nud_star(
        ...
        return ast.value_projection(
            left,
            right,
        )

    def _token_nud_filter(
        self,
        token,
    ):
        return self._token_led_filter(
            ast.identity()
        )

    def _token_nud_lbrace(
        self,
        token,
    ):
        return (
            self._parse_multi_select_hash()
        )

    def _token_nud_lparen(
        self,
        token,
    ):
        expression = (
    ...
    def _parse_index_expression(
        ...
        if (
            ...
            return (
                self._parse_slice_expression()
            )
        else:
            # Parse the syntax [number]
            node = ast.index(
                self._lookahead_token(
                    0
                )[
                    "value"
                ]
    ...
    def _token_led_and(
        ...
        return ast.and_expression(
            left,
            right,
        )

    def _token_led_lparen(
        self,
        left,
    ):
        if (
        ...
        function_node = ast.function_expression(
            name,
            args,
        )
        return function_node

    def _token_led_filter(
        self,
        left,
    ):
    ...
    def _project_if_slice(
        self,
        left,
        right,
    ):
        index_expr = ast.index_expression(
            [
                left,
                right,
            ]
    ...
    def _parse_comparator(
        ...
        return ast.comparator(
            comparator,
            left,
            right,
        )

    def _parse_multi_select_list(
        self,
    ):
        expressions = (
            []
        ...
        return ast.multi_select_list(
            expressions
        )

    def _parse_multi_select_hash(
        self,
    ):
        pairs = (
            []
        ...
        while True:
            key_token = self._lookahead_token(
                0
            )
            ...
            if (
                ...
                break
        return ast.multi_select_dict(
            nodes=pairs
        )

    def _parse_projection_rhs(
        self,
        binding_power,
    ):
    ...
    def _raise_parse_error_for_token(
        self,
        token,
        reason,
    ):
    ...
    def _raise_parse_error_maybe_eof(
        self,
        expected_type,
        token,
    ):
