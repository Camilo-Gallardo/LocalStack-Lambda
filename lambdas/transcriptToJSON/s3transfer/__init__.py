...
class MultipartUploader:
    ...
    def _upload_parts(
        ...
        upload_parts_extra_args = self._extra_upload_part_args(
            extra_args
        )
        parts = (
            []
        ...
        part_size = (
            self._config.multipart_chunksize
        )
        num_parts = int(
            math.ceil(
                self._os.get_file_size(
                    filename
                )
                / float(
                    part_size
    ...
    def _upload_one_part(
        ...
        open_chunk_reader = (
            self._os.open_file_chunk_reader
        )
...
class MultipartDownloader:
    def __init__(
        ...
        self._os = osutil
        self._executor_cls = executor_cls
        self._ioqueue = ShutdownQueue(
            self._config.max_io_queue
        )
    ...
    def download_file(
        ...
        with self._executor_cls(
            ...
            parts_future = controller.submit(
                download_parts_handler
            )
    ...
    def _download_range(
        ...
        try:
            ...
            max_attempts = (
                self._config.num_download_attempts
            )
            last_exception = None
            for i in range(
                max_attempts
            ...
            logger.debug(
                "EXITING _download_range for part: %s",
                part_index,
            )
...
class S3Transfer:
    ...
    def _put_object(
        ...
        open_chunk_reader = (
            self._osutil.open_file_chunk_reader
        )
        with open_chunk_reader(
            filename,
            0,
