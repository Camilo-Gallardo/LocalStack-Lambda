...
class BandwidthLimitedStream:
    ...
    def enable_bandwidth_limiting(
        self,
    ):
        """Enable bandwidth limiting on reads to the stream"""
        self._bandwidth_limiting_enabled = True

    def disable_bandwidth_limiting(
        self,
    ):
    ...
    def read(
        ...
        """
        if (
            not self._bandwidth_limiting_enabled
        ):
    ...
    def _consume_through_leaky_bucket(
        self,
    ):
        ...
        # worst.
        while (
            not self._transfer_coordinator.exception
        ):
            try:
                ...
                self._time_utils.sleep(
                    e.retry_time
                )
        else:
            raise self._transfer_coordinator.exception

    def signal_transferring(
        self,
    ):
    ...
    def close(
        ...
        if (
            self._bandwidth_limiting_enabled
            and self._bytes_seen
        ):
...
class LeakyBucket:
    def __init__(
        ...
        if (
            consumption_scheduler
            is None
        ):
            self._consumption_scheduler = (
    ...
    def consume(
        ...
        with self._lock:
            time_now = (
                self._time_utils.time()
            )
            if self._consumption_scheduler.is_scheduled(
                request_token
            ):
            ...
            elif self._projected_to_exceed_max_rate(
                amt,
                time_now,
            ):
                ...
                return self._release_requested_amt(
                    amt,
                    time_now,
                )

    def _projected_to_exceed_max_rate(
        self,
        amt,
        time_now,
    ):
    ...
    def _raise_request_exceeded_exception(
        self,
        amt,
        request_token,
        time_now,
    ...
    def _release_requested_amt(
        self,
        amt,
        time_now,
    ):
        self._rate_tracker.record_consumption_rate(
            amt,
            time_now,
        )
        return amt
...
class ConsumptionScheduler:
    ...
    def is_scheduled(
        ...
        return (
            token
            in self._tokens_to_scheduled_consumption
        )
    ...
    def process_scheduled_consumption(
        self,
        token,
    ):
        ...
        :param token: The token associated to the consumption
            request that is used to identify the request.
        """
        scheduled_retry = self._tokens_to_scheduled_consumption.pop(
            token
        )
