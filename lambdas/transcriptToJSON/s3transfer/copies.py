...
class CopySubmissionTask(
    ...
    def _submit(
        ...
        if (
            ...
            for (
                ...
                if (
                    param
                    in self.EXTRA_ARGS_TO_HEAD_ARGS_MAPPING
                ):
                    head_object_request[
                        self.EXTRA_ARGS_TO_HEAD_ARGS_MAPPING[
                            param
                        ]
                    ] = value
    ...
    def _submit_multipart_request(
        ...
        for (
            ...
            call_args.extra_args.items()
        ):
            if (
                param
                not in self.CREATE_MULTIPART_ARGS_BLACKLIST
            ):
                create_multipart_extra_args[
                    param
                ] = val
        ...
        for part_number in range(
            ...
            checksum_algorithm = call_args.extra_args.get(
                "ChecksumAlgorithm"
            )
            part_futures.append(
                self._transfer_coordinator.submit(
    ...
    def _get_head_object_request_from_copy_source(
        self,
        copy_source,
    ):
        if isinstance(
