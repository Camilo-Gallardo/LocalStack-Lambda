...
class DownloadOutputManager:
    ...
    def is_compatible(
        ...
        raise NotImplementedError(
            "must implement is_compatible()"
        )

    def get_download_task_tag(
        self,
    ):
    ...
    def get_fileobj_for_io_writes(
        ...
        """
        raise NotImplementedError(
            "must implement get_fileobj_for_io_writes()"
        )
    ...
    def _get_fileobj_from_filename(
        self,
        filename,
    ):
        f = DeferredOpenFile(
            filename,
            mode="wb",
            open_function=self._osutil.open,
        )
...
class DownloadFilenameOutputManager(
    ...
    def get_fileobj_for_io_writes(
        ...
        self._temp_filename = self._osutil.get_temp_filename(
            fileobj
        )
        self._temp_fileobj = (
            self._get_temp_fileobj()
        )
        return (
            self._temp_fileobj
        )
    ...
    def _get_temp_fileobj(
        self,
    ):
        f = self._get_fileobj_from_filename(
            self._temp_filename
        )
        self._transfer_coordinator.add_failure_cleanup(
...
class DownloadSeekableOutputManager(
    DownloadOutputManager
):
    @classmethod
    def is_compatible(
...
class DownloadNonSeekableOutputManager(
    DownloadOutputManager
):
    def __init__(
        self,
    ...
    def queue_file_io_task(
        ...
        with self._io_submit_lock:
            writes = self._defer_queue.request_writes(
                offset,
                data,
            )
            for write in writes:
...
class DownloadSpecialFilenameOutputManager(
    ...
    def get_fileobj_for_io_writes(
        ...
        self._fileobj = self._get_fileobj_from_filename(
            filename
        )
        return (
            self._fileobj
...
class DownloadSubmissionTask(
    ...
    def _get_download_output_manager_cls(
        ...
        for download_manager_cls in download_manager_resolver_chain:
            if download_manager_cls.is_compatible(
                fileobj,
                osutil,
            ):
    ...
    def _submit_download_request(
        ...
        progress_callbacks = get_callbacks(
            transfer_future,
            "progress",
        )
        ...
        get_object_tag = (
            download_output_manager.get_download_task_tag()
        )

        # Get the final io task to run once the download is complete.
        final_task = (
            download_output_manager.get_final_io_task()
        )
    ...
    def _get_final_io_task_submission_callback(
        ...
        final_task = (
            download_manager.get_final_io_task()
        )
        ...
        for i in range(
            ...
            try:
                ...
                for chunk in chunks:
                    ...
                    if (
                        not self._transfer_coordinator.done()
                    ):
                        self._handle_io(
                            download_output_manager,
                            fileobj,
...
class ImmediatelyWriteIOGetObjectTask(
    GetObjectTask
):
    ...
    def _handle_io(
        ...
        task = download_output_manager.get_io_write_task(
            fileobj,
            chunk,
            index,
        )
...
class CompleteDownloadNOOPTask(
    Task
):
...
class DeferQueue:
    ...
    def request_writes(
        ...
        while (
            ...
            self._pending_offsets.remove(
                next_write[
                    0
                ]
            )
            self._next_offset += len(
                next_write[
                    1
                ]
            )
