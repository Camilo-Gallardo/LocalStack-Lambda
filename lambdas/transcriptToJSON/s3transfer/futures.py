...
class TransferCoordinator:
    ...
    def __init__(
        ...
        self._associated_futures_lock = (
            threading.Lock()
        )
        self._done_callbacks_lock = (
            threading.Lock()
        )
        self._failure_cleanups_lock = (
            threading.Lock()
        )
    ...
    def set_status_to_queued(
        ...
        self._transition_to_non_done_state(
            "queued"
        )

    def set_status_to_running(
        self,
    ):
    ...
    def _transition_to_non_done_state(
        self,
        desired_state,
    ):
        with self._lock:
    ...
    def add_associated_future(
        self,
        future,
    ):
        """Adds a future to be associated with the TransferFuture"""
        with self._associated_futures_lock:
            self._associated_futures.add(
                future
            )

    def remove_associated_future(
        self,
        future,
    ):
        """Removes a future's association to the TransferFuture"""
        with self._associated_futures_lock:
            self._associated_futures.remove(
                future
            )
...
class BoundedExecutor:
    ...
    def submit(
        ...
        # tag.
        if tag:
            semaphore = self._tag_semaphores[
                tag
            ]

        # Call acquire on the semaphore.
        acquire_token = semaphore.acquire(
            task.transfer_id,
            block,
        )
