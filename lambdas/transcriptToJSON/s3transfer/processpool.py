...
def ignore_ctrl_c():
    original_handler = (
        _add_ignore_handler_for_interrupts()
    )
    yield
    signal.signal(
        signal.SIGINT,
...
class ProcessPoolDownloader:
    def __init__(
        ...
        self._download_request_queue = multiprocessing.Queue(
            1000
        )
    ...
    def download_file(
        ...
        transfer_id = (
            self._transfer_monitor.notify_new_transfer()
        )
        ...
        future = self._get_transfer_future(
            transfer_id,
            call_args,
        )
        return future
    ...
    def _get_transfer_future(
        ...
        future = ProcessPoolTransferFuture(
            monitor=self._transfer_monitor,
            meta=meta,
        )
        return future
    ...
    def _start_transfer_monitor_manager(
        ...
        logger.debug(
            "Starting the TransferMonitorManager."
        )
        self._manager = (
            TransferMonitorManager()
        )
        ...
        self._transfer_monitor = (
            self._manager.TransferMonitor()
        )

    def _start_submitter(
        self,
    ):
        logger.debug(
            "Starting the GetObjectSubmitter."
        )
    ...
    def _shutdown_transfer_monitor_manager(
        self,
    ):
        logger.debug(
            "Shutting down the TransferMonitorManager."
        )
        self._manager.shutdown()

    def _shutdown_submitter(
        self,
    ):
        logger.debug(
            "Shutting down the GetObjectSubmitter."
        )
        self._download_request_queue.put(
            SHUTDOWN_SIGNAL
        )
    ...
    def _shutdown_get_object_workers(
        self,
    ):
        logger.debug(
            "Shutting down the GetObjectWorkers."
        )
        for _ in (
            self._workers
        ):
...
class ProcessPoolTransferFuture(
    ...
    def result(
        ...
        try:
            return self._monitor.poll_for_result(
                self._meta.transfer_id
            )
...
class ClientFactory:
    def __init__(
        ...
        client_config = deepcopy(
            self._client_kwargs.get(
                "config",
                Config(),
            )
        )
        if (
            not client_config.user_agent_extra
        ):
...
class TransferMonitor:
    ...
    def poll_for_result(
        ...
        exception = self._transfer_states[
            transfer_id
        ].exception
        if exception:
            raise exception
    ...
    def notify_cancel_all_in_progress(
        self,
    ):
        for transfer_state in (
            self._transfer_states.values()
        ):
            if (
                not transfer_state.done
            ):
                transfer_state.exception = (
                    CancelledError()
                )
    ...
    def notify_expected_jobs_to_complete(
        self,
        transfer_id,
        num_jobs,
    ):
...
class TransferState:
    ...
    @property
    def jobs_to_complete(
        self,
    ):
        return (
            self._jobs_to_complete
        )
    ...
    def decrement_jobs_to_complete(
        self,
    ):
        with self._job_lock:
...
class TransferMonitorManager(
    BaseManager
):
    pass


TransferMonitorManager.register(
    "TransferMonitor",
    TransferMonitor,
)


class BaseS3TransferProcess(
...
class GetObjectSubmitter(
    BaseS3TransferProcess
):
    def __init__(
        self,
        transfer_config,
    ...
    def _do_run(
        ...
        while True:
            download_file_request = (
                self._download_request_queue.get()
            )
            if (
                download_file_request
                == SHUTDOWN_SIGNAL
    ...
    def _submit_get_object_jobs(
        ...
        if (
            size
            < self._transfer_config.multipart_threshold
        ):
    ...
    def _get_size(
        ...
        expected_size = (
            download_file_request.expected_size
        )
        if (
            expected_size
            is None
    ...
    def _submit_ranged_get_object_jobs(
        ...
        part_size = (
            self._transfer_config.multipart_chunksize
        )
    ...
    def _submit_get_object_job(
        self,
        **get_object_job_kwargs,
    ):
        self._worker_queue.put(
            GetObjectJob(
                **get_object_job_kwargs
...
class GetObjectWorker(
    ...
    def _do_run(
        ...
        while True:
            ...
            if (
                ...
                return
            if not self._transfer_monitor.get_exception(
                job.transfer_id
            ):
                self._run_get_object_job(
                    job
                )
            else:
                logger.debug(
    ...
    def _finalize_download(
        ...
        if self._transfer_monitor.get_exception(
            transfer_id
        ):
            self._osutil.remove_file(
                temp_filename
            )
        else:
            self._do_file_rename(
                transfer_id,
                temp_filename,
                filename,
            )
