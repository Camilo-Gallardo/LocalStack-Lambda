...
class CRTCredentialProviderAdapter:
    ...
    def __call__(
        ...
        credentials = (
            self._get_credentials().get_frozen_credentials()
        )
...
def create_s3_crt_client(
    ...
    * MB,
    ...
    if (
        ...
        client_tls_option = ClientTlsContext(
            tls_ctx_options
        )
        tls_connection_options = (
            client_tls_option.new_connection_options()
        )
...
class CRTTransferManager:
    ...
    def _cancel_transfers(
        ...
        for coordinator in (
            ...
            if (
                not coordinator.done()
            ):
                coordinator.cancel()

    def _finish_transfers(
        self,
    ):
        for coordinator in (
            self._future_coordinators
    ...
    def _submit_transfer(
        ...
        afterdone = AfterDoneHandler(
            coordinator
        )
        on_done_after_calls.append(
            afterdone
        )
        ...
        try:
            ...
            crt_s3_request = self._crt_s3_client.make_request(
                **crt_callargs
            )
        except Exception as e:
            coordinator.set_exception(
                e,
                True,
            )
            on_done = self._s3_args_creator.get_crt_callback(
                future,
                "done",
                after_subscribers=on_done_after_calls,
            )
            on_done(
                error=e
            )
        else:
            coordinator.set_s3_request(
                crt_s3_request
            )
        self._future_coordinators.append(
            coordinator
        )
...
class CRTTransferFuture(
    ...
    def set_exception(
        ...
        self._coordinator.set_exception(
            exception,
            override=True,
        )
...
class BotocoreCRTRequestSerializer(
    ...
    def _resolve_client_config(
        ...
        if (
            session.get_default_client_config()
        ):
        ...
        if (
            ...
            user_provided_config = client_kwargs[
                "config"
            ]
        ...
        if user_provided_config:
            client_config = user_provided_config.merge(
                client_config
            )
        client_kwargs[
            "config"
    ...
    def _crt_request_from_aws_request(
        self,
        aws_request,
    ):
        url_parts = urlsplit(
        ...
        crt_headers = awscrt.http.HttpHeaders(
            headers_list
        )
    ...
    def _convert_to_crt_http_request(
        self,
        ...
        if (
            ...
            crt_request.headers.remove(
                "Content-MD5"
            )
        return crt_request

    def _capture_http_request(
        self,
        request,
        **kwargs,
    ):
    ...
    def _get_botocore_http_request(
        self,
        client_method,
        call_args,
    ):
    ...
    def serialize_http_request(
        ...
        crt_request = self._convert_to_crt_http_request(
            botocore_http_request
        )
        return crt_request
...
class CRTTransferCoordinator:
    ...
    def set_done_callbacks_complete(
        self,
    ):
        self._done_event.set()

    def wait_until_on_done_callbacks_complete(
        self,
        timeout=None,
    ):
    ...
    def set_s3_request(
        ...
        self._s3_request = s3_request
        self._crt_future = (
            self._s3_request.finished_future
        )
...
class S3ClientArgsCreator:
    ...
    def get_make_request_args(
        ...
        if (
            ...
            data_len = self._os_utils.get_file_size(
                send_filepath
            )
            call_args.extra_args[
                "ContentLength"
        ...
        return {
            ...
            "on_progress": self.get_crt_callback(
                future,
                "progress",
            ),
        }
