...
class RuleSetStandardLibrary:
    ...
    def call_function(
        ...
        func_name = self.convert_func_name(
            func_signature[
                "fn"
            ]
        )
    ...
    def aws_partition(
        ...
        if (
            ...
            for partition in partitions:
                if self.is_partition_match(
                    value,
                    partition,
                ):
                    return self.format_partition_output(
                        partition
                    )

        # return the default partition if no matches were found
        aws_partition = partitions[
            0
        ]
        return self.format_partition_output(
            aws_partition
    ...
    def parse_url(
        ...
        return {
            ...
            "isIp": is_valid_ipv4_endpoint_url(
                value
            )
    ...
    def aws_is_virtual_hostable_s3_bucket(
        self,
        value,
        allow_subdomains,
    ):
...
class EndpointRule(
    ...
    def resolve_properties(
        ...
        elif rule_lib.is_template(
            properties
        ):
            return rule_lib.resolve_template_string(
                properties,
                scope_vars,
            )
...
class ParameterDefinition:
    ...
    def validate_input(
        ...
        if (
            ...
            msg = self.deprecated.get(
                "message"
            )
            since = self.deprecated.get(
                "since"
            )
            if msg:
                depr_str += f"\n{msg}"
...
class RuleSet:
    ...
    def __init__(
        ...
        self.parameters = self._ingest_parameter_spec(
            parameters
        )
        self.rules = [
            RuleCreator.create(
        ...
        self.rule_lib = RuleSetStandardLibrary(
            partitions
        )
        self.documentation = documentation

    def _ingest_parameter_spec(
        self,
        parameters,
    ):
        return {
...
class EndpointProvider:
    ...
    def resolve_endpoint(
        ...
        **input_parameters,
        ...
        endpoint = self.ruleset.evaluate(
            input_parameters
        )
        if (
            endpoint
            ...
            param_string = "\n".join(
                [
                    f"{key}: {value}"
                    for key, value in params_for_error.items()
                ]
            )
            raise EndpointResolutionError(
