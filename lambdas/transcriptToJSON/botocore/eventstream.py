...
class DecodeUtils:
    ...
    def unpack_byte_array(
        ...
        """
        uint_byte_format = DecodeUtils.UINT_BYTE_FORMAT[
            length_byte_size
        ]
        length = unpack(
            uint_byte_format,
...
class EventStreamHeaderParser:
    ...
    def _parse_value(
        ...
        value_unpacker = self._HEADER_TYPE_MAP[
            header_type
        ]
        (
            value,
...
class EventStreamBuffer:
    ...
    def _validate_prelude(
        ...
        if (
            ...
            raise InvalidHeadersLength(
                prelude.headers_length
            )

        if (
            prelude.payload_length
            > _MAX_PAYLOAD_LENGTH
        ):
    ...
    def _parse_prelude(
        ...
        prelude_bytes = self._data[
            :_PRELUDE_LENGTH
        ]
        (
            raw_prelude,
            _,
        ) = DecodeUtils.unpack_prelude(
            prelude_bytes
        )
    ...
    def _parse_headers(
        ...
        header_bytes = self._data[
            _PRELUDE_LENGTH : self._prelude.headers_end
        ]
        return self._header_parser.parse(
            header_bytes
        )
    ...
    def _parse_payload(
        ...
        payload_bytes = self._data[
            prelude.headers_end : prelude.payload_end
        ]
    ...
    def _parse_message_crc(
        ...
        crc_bytes = self._data[
            prelude.payload_end : prelude.total_length
        ]
        (
            message_crc,
            _,
        ) = DecodeUtils.unpack_uint32(
            crc_bytes
        )
        return message_crc
    ...
    def _validate_message_crc(
        ...
        message_crc = (
            self._parse_message_crc()
        )
        message_bytes = (
            self._parse_message_bytes()
        )
        _validate_checksum(
            message_bytes,
            message_crc,
    ...
    **Example:**
    ...
    def get_initial_response(
        ...
        try:
            ...
            event_type = initial_event.headers.get(
                ":event-type"
            )
            if (
                event_type
