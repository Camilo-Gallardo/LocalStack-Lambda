...
class ProxyConfiguration:
    ...
    def proxy_url_for(
        ...
        if proxy:
            proxy = self._fix_proxy_url(
                proxy
            )
        return proxy

    def proxy_headers_for(
        self,
        proxy_url,
    ):
        ...
        if (
            ...
            basic_auth = self._construct_basic_auth(
                username,
                password,
            )
            headers[
...
class URLLib3Session:
    ...
    def __init__(
        ...
        self._manager.pool_classes_by_scheme = (
            self._pool_classes_by_scheme
        )
    ...
    def _proxies_kwargs(
        ...
        **kwargs,
        ...
        return {
            k: v
            for k, v in proxies_kwargs.items()
            if v
            is not None
        }

    def _get_pool_manager_kwargs(
        self,
        **extra_kwargs,
    ):
        pool_manager_kwargs = {
    ...
    def _get_proxy_manager(
        ...
        if (
            proxy_url
            not in self._proxy_managers
        ):
            proxy_headers = self._proxy_config.proxy_headers_for(
                proxy_url
            )
            proxy_ssl_context = self._setup_proxy_ssl_context(
                proxy_url
            )
            ...
            proxy_manager.pool_classes_by_scheme = (
                self._pool_classes_by_scheme
            )
            self._proxy_managers[
                proxy_url
    ...
    def _setup_proxy_ssl_context(
        ...
        proxy_ca_bundle = proxies_settings.get(
            "proxy_ca_bundle"
        )
        proxy_cert = proxies_settings.get(
            "proxy_client_cert"
        )
        if (
            proxy_ca_bundle
        ...
        try:
            ...
            raise InvalidProxiesConfigError(
                error=e
            )
    ...
    def _chunked(
        ...
        transfer_encoding = ensure_bytes(
            transfer_encoding
        )
        return (
            transfer_encoding.lower()
            == b"chunked"
        )
    ...
    def send(
        ...
        try:
            ...
            conn = manager.connection_from_url(
                request.url
            )
            ...
            request_target = self._get_request_target(
                request.url,
                proxy_url,
            )
