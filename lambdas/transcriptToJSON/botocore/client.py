...
class ClientCreator:
    ...
    def create_client(
        ...
        service_name = first_non_none_response(
            responses,
            default=service_name,
        )
        service_model = self._load_service_model(
            service_name,
            api_version,
        )
        try:
            ...
            partition_data = self._loader.load_data(
                "partitions"
            )
        ...
        endpoint_bridge = ClientEndpointBridge(
            ...
            scoped_config,
            client_config,
            service_signing_name=service_model.metadata.get(
                "signingName"
            ),
    ...
    def create_client_class(
        ...
        return self._create_client_class(
            service_name,
            service_model,
        )
    ...
    def _create_client_class(
        ...
        py_name_to_operation_name = self._create_name_mapping(
            service_model
        )
        class_attributes[
            "_PY_TO_OP_NAME"
        ...
        service_id = (
            service_model.service_id.hyphenize()
        )
        self._event_emitter.emit(
            "creating-client-class.%s"
            % service_id,
            class_attributes=class_attributes,
            base_classes=bases,
        )
        class_name = get_service_module_name(
            service_model
        )
        cls = type(
            str(
    ...
    def _normalize_fips_region(
        ...
        if (
            ...
            if (
                ...
                config_use_fips_endpoint = Config(
    ...
    def _load_service_endpoints_ruleset(
        self,
        service_name,
        api_version=None,
    ):
    ...
    def _register_retries(
        self,
        client,
    ):
        retry_mode = client.meta.config.retries[
            "mode"
        ]
        if (
            retry_mode
            ...
            self._register_v2_standard_retries(
                client
            )
        elif (
            retry_mode
            ...
            self._register_v2_adaptive_retries(
                client
            )
        elif (
            retry_mode
            ...
            self._register_legacy_retries(
                client
            )

    def _register_v2_standard_retries(
        self,
        client,
    ):
        max_attempts = client.meta.config.retries.get(
            "total_max_attempts"
        )
        kwargs = {
            "client": client
    ...
    def _register_v2_adaptive_retries(
        self,
        client,
    ):
    ...
    def _register_legacy_retries(
        self,
        client,
    ):
        endpoint_prefix = (
            client.meta.service_model.endpoint_prefix
        )
        service_id = (
        ...
        original_config = self._loader.load_data(
            "_retry"
        )
        if (
            not original_config
        ...
        retries = self._transform_legacy_retries(
    ...
    def _get_retry_mode(
        ...
        return (
            config_store.get_config_variable(
                "retry_mode"
            )
            or "legacy"
        )

    def _register_endpoint_discovery(
        self,
        client,
        endpoint_url,
        config,
        ...
        if (
            client.meta.service_model.endpoint_discovery_operation
            is None
        ):
            return
        events = (
        ...
        service_id = (
            client.meta.service_model.service_id.hyphenize()
        )
        enabled = False
        if (
            config
            and config.endpoint_discovery_enabled
            is not None
        ):
            enabled = (
        ...
        enabled = self._normalize_endpoint_discovery_config(
            enabled
        )
        if (
            enabled
            and self._requires_endpoint_discovery(
                client,
                enabled,
            )
        ):
            ...
            manager = EndpointDiscoveryManager(
                client,
                always_discover=discover,
            )
            handler = EndpointDiscoveryHandler(
                manager
            )
            handler.register(
                events,
    ...
    def _normalize_endpoint_discovery_config(
        self,
        enabled,
    ):
        ...
        raise InvalidEndpointDiscoveryConfigurationError(
            config_value=enabled
        )

    def _requires_endpoint_discovery(
        self,
        client,
        enabled,
    ):
        if (
            ...
            return (
                client.meta.service_model.endpoint_discovery_required
            )
        return enabled
    ...
    def _register_s3_control_events(
        ...
        if (
            ...
            return
        S3ControlArnParamHandlerv2().register(
            client.meta.events
        )
    ...
    def _set_s3_presign_signature_version(
        ...
        regions = self._endpoint_resolver.get_available_endpoints(
            "s3",
            client_meta.partition,
        )
        if (
            client_meta.region_name
            != "aws-global"
            and client_meta.region_name
            not in regions
        ):
            return
        ...
        signature_versions = endpoint[
            "signatureVersions"
        ]
        if (
            "s3"
            not in signature_versions
        ):
            return
        ...
        client_meta.events.register(
            "choose-signer.s3",
            self._default_s3_presign_to_sigv2,
        )

    def _default_s3_presign_to_sigv2(
        self,
        signature_version,
        **kwargs,
    ):
        ...
        """
        if signature_version.startswith(
            "v4a"
        ):
            return
    ...
    def _create_api_method(
        ...
        docstring = ClientMethodDocstring(
            ...
            example_prefix="response = client.%s"
            % py_operation_name,
...
class ClientEndpointBridge:
    ...
    def resolve(
        ...
        region_name = self._check_default_region(
            service_name,
            region_name,
        )
    ...
    def resolver_uses_builtin_data(
        self,
    ):
        return (
            self.endpoint_resolver.uses_builtin_data
        )

    def _check_default_region(
        self,
        service_name,
        region_name,
    ):
        ...
        if (
            self.client_config
            and self.client_config.region_name
            is not None
        ):
            return (
                self.client_config.region_name
            )
    ...
    def _create_endpoint(
        ...
        signing_name = self._resolve_signing_name(
            service_name,
            resolved,
        )
    ...
    def _resolve_endpoint_variant_config_var(
        self,
        config_var,
    ):
        client_config = (
        ...
        if (
            ...
            # Check config store
            config_val = self.config_store.get_config_variable(
                config_var
            )
        return config_val

    def _resolve_use_dualstack_endpoint(
        self,
        service_name,
    ):
        s3_dualstack_mode = self._is_s3_dualstack_mode(
            service_name
        )
        if (
            s3_dualstack_mode
    ...
    def _is_s3_dualstack_mode(
        self,
        service_name,
    ):
        if (
            service_name
            not in self._DUALSTACK_CUSTOMIZED_SERVICES
        ):
            return None
        ...
        if (
            ...
            enabled = self.scoped_config.get(
                "s3",
                {},
            ).get(
                "use_dualstack_endpoint"
    ...
    def _resolve_signature_version(
        ...
        potential_versions = resolved.get(
            "signatureVersions",
            [],
        )
        if (
            self.service_signature_version
            is not None
...
class BaseClient:
    ...
    def _register_handlers(
        ...
        service_id = (
            self.meta.service_model.service_id.hyphenize()
        )
    ...
    def _make_api_call(
        ...
        if (
            operation_model.deprecated
        ):
            logger.debug(
                "Warning: %s.%s() is deprecated",
                service_name,
                operation_name,
            )
        request_context = {
        ...
        if (
            ...
            error_info = parsed_response.get(
                "Error",
                {},
            )
            ...
            error_class = self.exceptions.from_code(
                error_code
            )
            raise error_class(
                parsed_response,
    ...
    def _make_request(
        ...
        try:
            return self._endpoint.make_request(
                operation_model,
                request_dict,
            )
    ...
    def _convert_to_request_dict(
        ...
        if (
            not self._client_config.inject_host_prefix
        ):
            request_dict.pop(
                "host_prefix",
                None,
        ...
        if set_user_agent_header:
            user_agent = (
                self._user_agent_creator.to_string()
            )
        else:
            user_agent = None
        prepare_request_dict(
    ...
    def _emit_api_params(
        ...
        api_params = first_non_none_response(
            responses,
            default=api_params,
        )
...
class ClientMeta:
    ...
    @property
    def method_to_api_mapping(
        self,
    ):
        return (
            self._method_to_api_mapping
        )
...
def _get_configured_signature_version(
    ...
    if (
        client_config
        and client_config.signature_version
        is not None
    ):
        return (
    ...
    if (
        ...
        service_config = scoped_config.get(
            service_name
        )
        if (
            service_config
