...
class ClientArgsCreator:
    ...
    def get_client_args(
        ...
        service_name = final_args[
            "service_name"
        ]  # noqa
        parameter_validation = final_args[
            "parameter_validation"
        ]
        endpoint_config = final_args[
            "endpoint_config"
        ]
        protocol = final_args[
            "protocol"
        ]
        config_kwargs = final_args[
            "config_kwargs"
        ]
        s3_config = final_args[
            "s3_config"
        ]
        partition = endpoint_config[
            "metadata"
        ].get(
            "partition",
            None,
        )
        socket_options = final_args[
            "socket_options"
        ]
        configured_endpoint_url = final_args[
            "configured_endpoint_url"
        ]
        signing_region = endpoint_config[
            "signing_region"
        ]
        endpoint_region_name = endpoint_config[
            "region_name"
        ]

        event_emitter = copy.copy(
            self._event_emitter
        )
        ...
        response_parser = botocore.parsers.create_parser(
            protocol
        )
    ...
    def compute_client_args(
        ...
        protocol = service_model.metadata[
            "protocol"
        ]
        parameter_validation = True
        if (
            client_config
            and not client_config.parameter_validation
        ):
            parameter_validation = False
        elif scoped_config:
            raw_value = scoped_config.get(
                "parameter_validation"
            )
            if (
                raw_value
                ...
                parameter_validation = ensure_boolean(
                    raw_value
                )

        s3_config = self.compute_s3_config(
            client_config
        )
        ...
        endpoint_variant_tags = endpoint_config[
            "metadata"
        ].get(
            "tags",
            [],
        ...
        self._compute_retry_config(
            config_kwargs
        )
        self._compute_connect_timeout(
            config_kwargs
        )
        self._compute_user_agent_appid_config(
            config_kwargs
        )
        self._compute_request_compression_config(
            config_kwargs
        )
        s3_config = self.compute_s3_config(
            client_config
        ...
        is_s3_service = self._is_s3_service(
            service_name
        )
    ...
    def _compute_configured_endpoint_url(
        ...
        if self._ignore_configured_endpoint_urls(
            client_config
        ):
            logger.debug(
    ...
    def _ignore_configured_endpoint_urls(
        self,
        client_config,
    ):
        if (
            client_config
            and client_config.ignore_configured_endpoint_urls
            is not None
        ):
            return (
    ...
    def compute_s3_config(
        self,
        client_config,
    ):
        s3_configuration = self._config_store.get_config_variable(
            "s3"
        )
        ...
        if (
            ...
            if (
                ...
                if (
                    ...
                    s3_configuration = (
                        s3_configuration.copy()
                    )
                    s3_configuration.update(
                        client_config.s3
                    )
    ...
    def _compute_endpoint_config(
        ...
        if (
            ...
            return self._compute_sts_endpoint_config(
                **resolve_endpoint_kwargs
            )
    ...
    def _compute_s3_endpoint_config(
        ...
        **resolve_endpoint_kwargs,
        ...
        endpoint_config = self._resolve_endpoint(
            **resolve_endpoint_kwargs
        )
    ...
    def _should_force_s3_global(
        ...
        if (
            s3_config
            and "us_east_1_regional_endpoint"
            in s3_config
        ):
            s3_regional_config = s3_config[
                "us_east_1_regional_endpoint"
            ]
            self._validate_s3_regional_config(
                s3_regional_config
            )
        ...
        return (
            s3_regional_config
            == "legacy"
            and is_global_region
        )

    def _validate_s3_regional_config(
        self,
        config_val,
    ):
        if (
    ...
    def _set_region_if_custom_s3_endpoint(
        ...
        if (
            ...
            endpoint = endpoint_bridge.resolve(
                "s3"
            )
            endpoint_config[
                "signing_region"
    ...
    def _compute_sts_endpoint_config(
        self,
        **resolve_endpoint_kwargs,
    ):
        endpoint_config = self._resolve_endpoint(
    ...
    def _should_set_global_sts_endpoint(
        ...
        if (
            ...
            return False
        return (
            self._get_sts_regional_endpoints_config()
            == "legacy"
            and region_name
            in LEGACY_GLOBAL_STS_REGIONS
        )

    def _get_sts_regional_endpoints_config(
        self,
    ):
        ...
        if (
            not sts_regional_endpoints_config
        ):
    ...
    def _compute_socket_options(
        ...
        client_keepalive = (
            client_config
            and client_config.tcp_keepalive
        )
        scoped_keepalive = (
            scoped_config
            and self._ensure_boolean(
                scoped_config.get(
                    "tcp_keepalive",
                    False,
                )
    ...
    def _compute_retry_config(
        self,
        config_kwargs,
    ):
        self._compute_retry_max_attempts(
            config_kwargs
        )
        self._compute_retry_mode(
            config_kwargs
        )

    def _compute_retry_max_attempts(
        self,
        config_kwargs,
    ):
        ...
        max_attempts = self._config_store.get_config_variable(
            "max_attempts"
        )
        if (
            max_attempts
    ...
    def _compute_retry_mode(
        ...
        if (
            ...
            return
        retry_mode = self._config_store.get_config_variable(
            "retry_mode"
        )
        if (
            retry_mode
    ...
    def _compute_connect_timeout(
        ...
        connect_timeout = config_kwargs.get(
            "connect_timeout"
        )
        if (
            connect_timeout
    ...
    def _compute_request_compression_config(
        self,
        config_kwargs,
    ):
        min_size = config_kwargs.get(
            "request_min_compression_size_bytes"
        )
        disabled = config_kwargs.get(
            "disable_request_compression"
        )
        if (
            min_size
        ...
        # config object
        min_size = self._validate_min_compression_size(
            min_size
        )
        config_kwargs[
            "request_min_compression_size_bytes"
    ...
    def _validate_min_compression_size(
        self,
        min_size,
    ):
        ...
        if (
            ...
            if (
                not min_allowed_min_size
                <= min_size
                <= max_allowed_min_size
            ):
                msg = (
    ...
    def _build_endpoint_resolver(
        ...
        if (
            ...
            client_context = (
                client_config.client_context_params
                or {}
            )
        else:
            client_context = (
    ...
    def compute_endpoint_resolver_builtin_defaults(
        ...
        # endpoint resolution is provided to EndpointRulesetResolver.
            ...
            not endpoint_bridge.resolver_uses_builtin_data()
        ):
    ...
    def _compute_user_agent_appid_config(
        self,
        config_kwargs,
    ):
        user_agent_appid = config_kwargs.get(
            "user_agent_appid"
        )
        if (
            user_agent_appid
