...
def build_retry_config(
    ...
    service_config = retry_model.get(
        endpoint_prefix,
        {},
    )
    resolve_references(
...
def _merge_client_retry_config(
    retry_config,
    client_retry_config,
):
    max_retry_attempts_override = client_retry_config.get(
        "max_attempts"
    )
    if (
