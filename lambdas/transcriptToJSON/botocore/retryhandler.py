...
def create_checker_from_retry_config(
    ...
    if (
        ...
        for key in policies:
            current_config = policies[
                key
            ]
            checkers.append(
                _create_single_checker(
                    current_config
                )
            )
...
def _create_single_response_checker(
    response,
):
    if (
        "service_error_code"
...
def _extract_retryable_exception(
    config,
):
    applies_when = config[
        "applies_when"
        ...
        **kwargs,
        ...
        if isinstance(
            ...
            checker_kwargs.update(
                {
                    "retries_context": retries_context
                }
            )
...
class BaseChecker:
    ...
    def __call__(
        ...
        if (
            ...
            raise ValueError(
                "Both response and caught_exception are None."
            )
...
class CRC32Checker(
    ...
    def _check_response(
        ...
        expected_crc = http_response.headers.get(
            self._header_name
        )
        if (
            expected_crc
