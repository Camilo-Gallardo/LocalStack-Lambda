...
def _host_from_url(
    ...
    host = (
        url_parts.hostname
    )  # urlsplit's hostname is always lowercase
    if is_valid_ipv6_endpoint_url(
        url
    ):
        host = f"[{host}]"
    default_ports = {
...
class SigV2Auth(
    ...
    def calc_signature(
        ...
        logger.debug(
            "Calculating signature using v2 auth."
        )
        split = urlsplit(
            request.url
        )
...
class SigV4Auth(
    ...
    def headers_to_sign(
        ...
        for (
            ...
            if (
                lname
                not in SIGNED_HEADERS_BLACKLIST
            ):
                header_map[
                    lname
                ] = value
    ...
    def canonical_query_string(
        ...
        if (
            ...
            return self._canonical_query_string_params(
                request.params
            )
        else:
            return self._canonical_query_string_url(
                urlsplit(
                    request.url
                )
            )

    def _canonical_query_string_params(
        self,
        params,
    ):
    ...
    def _canonical_query_string_url(
        self,
        parts,
    ):
    ...
    def canonical_headers(
        ...
        for key in sorted_header_names:
            value = ",".join(
                ...
                for v in headers_to_sign.get_all(
                    key
                )
            )
            headers.append(
                f"{key}:{ensure_unicode(value)}"
            )
        return "\n".join(
            headers
        )
    ...
    def payload(
        ...
        if self._is_streaming_checksum_payload(
            request
        ):
            return STREAMING_UNSIGNED_PAYLOAD_TRAILER
        elif not self._should_sha256_sign_payload(
            request
        ):
    ...
    def canonical_request(
        ...
        if (
            ...
            body_checksum = request.headers[
                "X-Amz-Content-SHA256"
            ]
        else:
            body_checksum = self.payload(
                request
            )
        cr.append(
            body_checksum
    ...
    def add_auth(
        ...
        self._modify_request_before_signing(
            request
        )
    ...
    def _modify_request_before_signing(
        self,
        request,
    ):
        if (
        ...
        self._set_necessary_date_headers(
            request
        )
        if (
            self.credentials.token
    ...
    def _set_necessary_date_headers(
        self,
        request,
    ):
...
class S3SigV4Auth(
    ...
    def _modify_request_before_signing(
        ...
        super()._modify_request_before_signing(
            request
        )
        if (
            "X-Amz-Content-SHA256"
    ...
    def _should_sha256_sign_payload(
        ...
        client_config = request.context.get(
            "client_config"
        )
        s3_config = getattr(
            client_config,
        ...
        # checks.
        return super()._should_sha256_sign_payload(
            request
        )
...
class SigV4QueryAuth(
    ...
    def _modify_request_before_signing(
        ...
        auth_params = {
            ...
            "X-Amz-Credential": self.scope(
                request
            ),
            "X-Amz-Date": request.context[
                "timestamp"
            ],
...
class HmacV1Auth(
    ...
    def canonical_standard_headers(
        self,
        headers,
    ):
        interesting_headers = [
            ...
            if (
                ...
                if lk.startswith(
                    ...
                    custom_headers[
                        ...
                        v.strip()
                        for v in headers.get_all(
                            key
                        )
                    )
        sorted_header_keys = sorted(
            custom_headers.keys()
        )
        for key in sorted_header_keys:
            hoi.append(
                f"{key}:{custom_headers[key]}"
            )
        return "\n".join(
            hoi
        )
    ...
    def canonical_string(
        ...
        cs += (
            self.canonical_standard_headers(
                headers
            )
            + "\n"
        )
...
class HmacV1QueryAuth(
    ...
    def _inject_signature(
        ...
        # query string
        new_query_string = percent_encode_sequence(
            query_dict
        )
