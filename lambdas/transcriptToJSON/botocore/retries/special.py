...
# available.
class RetryIDPCommunicationError(
    BaseRetryableChecker
):
    _SERVICE_NAME = "sts"
    ...
    def is_retryable(
        ...
        service_name = (
            context.operation_model.service_model.service_name
        )
        if (
            service_name
            != self._SERVICE_NAME
...
class RetryDDBChecksumError(
    ...
    def is_retryable(
        ...
        if (
            ...
            return False
        checksum = context.http_response.headers.get(
