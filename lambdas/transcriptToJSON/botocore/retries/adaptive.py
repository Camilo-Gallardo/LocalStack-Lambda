...
class ClientRateLimiter:
    ...
    def on_receiving_response(
        self,
        **kwargs
    ):
        measured_rate = (
            self._rate_clocker.record()
        )
        timestamp = (
            self._clock.current_time()
        )
        with self._lock:
            if not self._throttling_detector.is_throttling_error(
                **kwargs
            ):
                new_rate = self._rate_adjustor.success_received(
                    timestamp
                )
            else:
                if (
...
class RateClocker:
    ...
    def __init__(
        ...
        self._last_bucket = math.floor(
            self._clock.current_time()
        )
        self._time_bucket_scale = (
            1
            / self._TIME_BUCKET_RANGE
        )
        self._count = 0
        self._lock = (
            threading.Lock()
