...
def register_retry_handler(
    ...
    client.meta.events.register(
        f"after-call.{service_event_name}",
...
class RetryHandler:
    ...
    def needs_retry(
        ...
        **kwargs,
        ...
        retry_delay = None
        context = self._retry_event_adapter.create_retry_context(
            **kwargs
        )
        if self._retry_policy.should_retry(
            ...
            if self._retry_quota.acquire_retry_quota(
                context
            ):
                retry_delay = self._retry_policy.compute_retry_delay(
                    context
                )
                logger.debug(
...
class RetryEventAdapter:
    ...
    def create_retry_context(
        ...
        **kwargs,
        ...
        context = RetryContext(
            attempt_number=kwargs[
                "attempts"
            ],
            operation_model=kwargs[
                "operation"
            ],
            http_response=http_response,
            parsed_response=parsed_response,
            caught_exception=kwargs[
                "caught_exception"
            ],
    ...
    def adapt_retry_response_from_context(
        self,
        context,
    ):
...
class RetryContext:
    ...
    def get_error_code(
        ...
        if (
            ...
            return
        error = self.parsed_response.get(
            "Error",
            {},
        )
        if not isinstance(
...
class RetryPolicy:
    ...
    def should_retry(
        ...
        return self._retry_checker.is_retryable(
            context
        )
    ...
    def compute_retry_delay(
        ...
        return self._retry_backoff.delay_amount(
            context
        )
...
class MaxAttemptsChecker(
    ...
    def is_retryable(
        ...
        retries_context = context.request_context.get(
            "retries"
        )
        if retries_context:
            retries_context[
        ...
        if (
            not under_max_attempts
        ):
            logger.debug(
                "Max attempts of %s reached.",
                self._max_attempts,
            )
...
class TransientRetryableChecker(
    BaseRetryableChecker
):
    ...
    def is_retryable(
        ...
        if (
            context.get_error_code()
            in self._transient_error_codes
        ):
            return True
        if (
...
class ThrottledRetryableChecker(
    BaseRetryableChecker
):
    ...
    def is_retryable(
        ...
        return (
            context.get_error_code()
            in self._throttled_error_codes
        )


class ModeledRetryableChecker(
    BaseRetryableChecker
):
    ...
    def is_retryable(
        ...
        if (
            ...
            return False
        return (
            self._error_detector.detect_error_type(
                context
            )
            is not None
        )
...
class ModeledRetryErrorDetector:
    ...
    def detect_error_type(
        ...
        if (
            op_model
            is None
            or not op_model.error_shapes
        ):
            return
        for shape in (
            op_model.error_shapes
        ):
            if (
                shape.metadata.get(
                    "retryable"
...
class ThrottlingErrorDetector:
    ...
    def is_throttling_error(
        ...
        **kwargs,
        ...
        if self._fixed_error_code_detector.is_retryable(
            context
        ):
            return True
        error_type = self._modeled_error_detector.detect_error_type(
            context
        )
        return (
            error_type
            == self._modeled_error_detector.THROTTLING_ERROR
        )


class StandardRetryConditions(
    BaseRetryableChecker
):
    ...
    def __init__(
        ...
        self._max_attempts_checker = MaxAttemptsChecker(
            max_attempts
        )
    ...
    def is_retryable(
        ...
        return self._max_attempts_checker.is_retryable(
            context
        ) and self._additional_checkers.is_retryable(
            context
        )


class OrRetryChecker(
    BaseRetryableChecker
):
    def __init__(
        self,
...
class RetryQuotaChecker:
    ...
    def acquire_retry_quota(
        ...
        if self._is_timeout_error(
            ...
            capacity_amount = (
                self._TIMEOUT_RETRY_REQUEST
            )
        else:
            capacity_amount = (
                self._RETRY_COST
            )
        success = self._quota.acquire(
            capacity_amount
        )
        if success:
