...
class InstanceMetadataFetcher(IMDSFetcher):
    ...
    def retrieve_iam_role_credentials(self):
        try:
            ...
            logger.debug(
                "Max number of attempts exceeded (%s) when " "attempting to retrieve data from metadata service.",
                self._num_attempts,
            )
    ...
    def _evaluate_expiration(self, credentials):
        ...
        try:
            ...
            if current_time >= extension_time:
                ...
                logger.info(f"Attempting credential expiration extension due to a " f"credential service availability issue. A refresh of " f"these credentials will be attempted again within " f"the next {refresh_interval_with_jitter/60:.0f} minutes.")
        except ValueError:
...
class IMDSRegionProvider:
    ...
    def _create_fetcher(self):
        ...
        imds_config = {
            "ec2_metadata_service_endpoint": self._session.get_config_variable("ec2_metadata_service_endpoint"),
...
class S3RegionRedirector:
    ...
    def __init__(self, endpoint_bridge, client, cache=None):
        ...
        warnings.warn(
            "The S3RegionRedirector class has been deprecated for a new " "internal replacement. A future version of botocore may remove " "this class.",
...
class ArnParser:
    def parse_arn(self, arn):
        ...
        if len(arn_parts) < 6:
            raise InvalidArnException("Provided ARN: %s must be of the format: " "arn:partition:service:region:account:resource" % arn)
        return {
...
class S3ArnParamHandler:
    _RESOURCE_REGEX = re.compile(r"^(?P<resource_type>accesspoint|outpost)[/:](?P<resource_name>.+)$")
    _OUTPOST_RESOURCE_REGEX = re.compile(r"^(?P<outpost_name>[a-zA-Z0-9\-]{1,63})[/:]accesspoint[/:]" r"(?P<accesspoint_name>[a-zA-Z0-9\-]{1,63}$)")
...
class S3ControlArnParamHandler:
    ...
    def __init__(self, arn_parser=None):
        ...
        if arn_parser is None:
            self._arn_parser = ArnParser()
        warnings.warn(
            "The S3ControlArnParamHandler class has been deprecated for a new " "internal replacement. A future version of botocore may remove " "this class.",
            category=FutureWarning,
        )
    ...
    def _override_account_id_param(self, params, arn_details):
        ...
        if "AccountId" in params and params["AccountId"] != account_id:
            error_msg = ("Account ID in arn does not match the AccountId parameter " 'provided: "%s"') % params["AccountId"]
            raise UnsupportedS3ControlArnError(
...
class ContainerMetadataFetcher:
    ...
    def _validate_allowed_url(self, full_url):
        ...
        if not is_whitelisted_host:
            raise ValueError("Unsupported host '%s'.  Can only " "retrieve metadata from these hosts: %s" % (parsed.hostname, ", ".join(self._ALLOWED_HOSTS)))
    ...
    def _get_response(self, full_url, headers, timeout):
        try:
            ...
            if response.status_code != 200:
                raise MetadataRetrievalError(error_msg=("Received non 200 response (%s) from ECS metadata: %s") % (response.status_code, response_text))
            try:
            ...
            error_msg = "Received error when attempting to retrieve " "ECS metadata: %s" % e
            raise MetadataRetrievalError(error_msg=error_msg)
...
class EventbridgeSignerSetter:
    ...
    def _get_global_endpoint(self, endpoint, endpoint_variant_tags=None):
        ...
        dns_suffix = resolver.get_partition_dns_suffix(partition, endpoint_variant_tags=endpoint_variant_tags)
        if dns_suffix is None:
            dns_suffix = self._DEFAULT_DNS_SUFFIX
...
class JSONFileCache:
    ...
    def __setitem__(self, cache_key, value):
        ...
        try:
            ...
            raise ValueError(f"Value cannot be cached, must be " f"JSON serializable: {value}")
