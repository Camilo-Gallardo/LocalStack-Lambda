...
from botocore.retryhandler import (
    EXCEPTION_MAP as RETRYABLE_EXCEPTIONS,
)
...
class Monitor:
    ...
    def capture(
        ...
        **payload,
        ...
        """
        try:
            monitor_event = self._adapter.feed(
                event_name,
                payload,
            )
            if monitor_event:
...
class MonitorEventAdapter:
    ...
    def _handle_before_parameter_build(
        self,
        model,
        context,
        **kwargs,
    ...
    def _handle_request_created(
        ...
        **kwargs,
        ...
        new_attempt_event = context[
            "current_api_call_event"
        ].new_api_call_attempt(
            timestamp=self._get_current_time()
        )
        new_attempt_event.request_headers = (
            request.headers
        )
        new_attempt_event.url = (
            request.url
        )
        context[
            "current_api_call_attempt_event"
        ] = new_attempt_event
    ...
    def _handle_response_received(
        ...
        **kwargs,
        ...
        attempt_event = context.pop(
            "current_api_call_attempt_event"
        )
        attempt_event.latency = self._get_latency(
            attempt_event
        )
        if (
            parsed_response
            ...
            attempt_event.parsed_error = parsed_response.get(
                "Error"
            )
        else:
    ...
    def _handle_after_call(
        ...
        **kwargs,
        ...
        return self._complete_api_call(
            context
        )

    def _handle_after_call_error(
        self,
        context,
        exception,
        **kwargs,
        ...
        context[
            "current_api_call_event"
        ].retries_exceeded = self._is_retryable_exception(
            exception
        )
        return self._complete_api_call(
            context
    ...
    def _complete_api_call(
        self,
        context,
    ):
        call_event = context.pop(
            "current_api_call_event"
        )
        call_event.latency = self._get_latency(
            call_event
        )
        return call_event
...
class CSMSerializer:
    ...
    def serialize(
        ...
        """
        event_dict = self._get_base_event_dict(
            event
        )
        ...
        for attr in (
            self._SERIALIZEABLE_EVENT_PROPERTIES
        ):
            value = getattr(
                event,
                attr,
    ...
    def _serialize_attempts(
        ...
        **kwargs,
        ...
        if attempts:
            self._add_fields_from_last_attempt(
                event_dict,
                attempts[
                    -1
                ],
    ...
    def _add_fields_from_last_attempt(
        self,
        event_dict,
        last_attempt,
    ):
    ...
    def _serialize_response_headers(
        ...
        **kwargs,
        ...
        for (
            ...
            self._RESPONSE_HEADERS_TO_EVENT_ENTRIES.items()
        ):
            if (
                header
                in response_headers
    ...
    def _get_access_key(
        ...
        auth_val = self._get_auth_value(
            request_headers
        )
        (
            _,
            auth_match,
        ) = self._get_auth_match(
            auth_val
        )
        return auth_match.group(
            "access_key"
        )
...
class SocketPublisher:
    ...
    def publish(
        ...
        """
        serialized_event = self._serializer.serialize(
            event
        )
        if (
            len(
