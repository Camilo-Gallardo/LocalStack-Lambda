...
def _allowlist_generate_presigned_url(
    method_name,
    service_name,
    **kwargs,
):
...
class ClientDocumenter:
    ...
    def document_client(
        ...
        self._add_class_signature(
            section
        )
        client_methods = (
            self._get_client_methods()
        )
        self._add_client_intro(
            section,
            client_methods,
        )
        self._add_client_methods(
            client_methods
        )
    ...
    def _get_client_methods(
        ...
        client_methods = get_instance_public_methods(
            self._client
        )
        return self._filter_client_methods(
            client_methods
        )

    def _filter_client_methods(
        self,
        client_methods,
    ):
        filtered_methods = (
    ...
    def _filter_client_method(
        self,
        **kwargs,
    ):
        # Apply each filter to the method
        for filter in (
            self._CLIENT_METHODS_FILTERS
        ):
    ...
    def _add_client_intro(
        self,
        section,
        client_methods,
    ):
        section = section.add_new_section(
            "intro"
        )
        ...
        self._add_client_creation_example(
            section
        )
        ...
        for method_name in sorted(
            ...
            section.style.tocitem(
                f"{self._service_name}/client/{method_name}"
            )

    def _add_class_signature(
        self,
        section,
    ):
    ...
    def _add_client_creation_example(
        self,
        section,
    ):
    ...
    def _add_client_methods(
        ...
        for method_name in sorted(
            ...
            method_doc_structure.write_to_file(
                client_dir_path,
                method_name,
            )

    def _add_client_method(
        self,
        section,
        method_name,
        method,
    ):
        breadcrumb_section = section.add_new_section(
            "breadcrumb"
        )
        ...
        breadcrumb_section.write(
            f" / Client / {method_name}"
        )
        section.add_title_section(
            method_name
        )
        method_section = section.add_new_section(
            method_name,
            context={
                "qualifier": f"{self._client_class_name}.Client."
            },
        )
    ...
    def _is_custom_method(
        self,
        method_name,
    ):
        return (
            method_name
            not in self._client.meta.method_to_api_mapping
        )
    ...
    def _add_method_exceptions_list(
        self,
        section,
        operation_model,
    ):
        error_section = section.add_new_section(
            "exceptions"
        )
        error_section.style.new_line()
        error_section.style.bold(
            "Exceptions"
        )
        error_section.style.new_line()
        for error in (
            operation_model.error_shapes
        ):
            class_name = f"{self._client_class_name}.Client.exceptions.{error.name}"
            error_section.style.li(
                ":py:class:`%s`"
                % class_name
            )
    ...
    def _add_model_driven_method(
        ...
        operation_name = self._client.meta.method_to_api_mapping[
            method_name
        ]
        ...
        if (
            ...
            self._add_method_exceptions_list(
                section,
                operation_model,
            )

        # Add the shared examples
        shared_examples = self._shared_examples.get(
            operation_name
        )
        if shared_examples:
            document_shared_examples(
...
class ClientExceptionsDocumenter:
    ...
    def _exception_class_name(
        self,
        shape,
    ):
        return f"{self._client_class_name}.Client.exceptions.{shape.name}"

    def _add_exceptions_list(
        self,
        section,
    ):
        error_shapes = (
            self._client.meta.service_model.error_shapes
        )
        if (
            not error_shapes
        ):
    ...
    def _add_exception_classes(
        ...
        for shape in (
            ...
            self._add_exception_class(
                exception_doc_structure,
                shape,
            )
    ...
    def _add_exception_class(
        ...
        breadcrumb_section.write(
            f" / Client / exceptions / {shape.name}"
        )
        section.add_title_section(
            shape.name
        )
        class_section = section.add_new_section(
            shape.name
        )
        class_name = self._exception_class_name(
            shape
        )
        class_section.style.start_sphinx_py_class(
            class_name=class_name
        )
        self._add_top_level_documentation(
            class_section,
            shape,
        )
        self._add_exception_catch_example(
            class_section,
            shape,
        )
        self._add_response_attr(
            class_section,
            shape,
        )
        class_section.style.end_sphinx_py_class()

    def _add_top_level_documentation(
        self,
        section,
        shape,
    ):
    ...
    def _add_exception_catch_example(
        self,
        section,
        shape,
    ):
        ...
        section.style.new_line()
        section.write(
            "except client.exceptions.%s as e:"
            % shape.name
        )
        section.style.indent()
        section.style.new_line()
    ...
    def _add_response_attr(
        self,
        section,
        shape,
    ):
        response_section = section.add_new_section(
            "response"
        )
        response_section.style.start_sphinx_py_attr(
            "response"
        )
        self._add_response_attr_description(
            response_section
        )
        ...
        self._add_response_params(
            response_section,
            shape,
        )
        response_section.style.end_sphinx_py_attr()

    def _add_response_attr_description(
        self,
        section,
    ):
        section.style.new_line()
    ...
    def _add_response_example(
        ...
        example_section = section.add_new_section(
            "syntax"
        )
        example_section.style.new_line()
        example_section.style.bold(
            "Syntax"
        )
        ...
        documenter.document_example(
            ...
            shape,
            include=[
                self._GENERIC_ERROR_SHAPE
            ],
        )

    def _add_response_params(
        self,
        section,
        shape,
    ):
        params_section = section.add_new_section(
            "Structure"
        )
        params_section.style.new_line()
        params_section.style.bold(
            "Structure"
        )
...
class ClientContextParamsDocumenter:
    ...
    def _add_context_params_list(
        ...
        section.writeln(
            f"The available {sn} client context params are:"
        )
        for param in (
            ...
            section.write(
                f"* {name} ({param.type}) - {param.documentation}"
            )
