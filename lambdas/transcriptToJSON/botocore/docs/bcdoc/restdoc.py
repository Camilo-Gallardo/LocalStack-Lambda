...
from botocore.docs.bcdoc.docstringparser import (
    DocStringParser,
)
from botocore.docs.bcdoc.style import (
    ReSTStyle,
)
    ...
    def getvalue(
        ...
        if (
            ...
            for (
                ...
                self.style.link_target_definition(
                    refname,
                    link,
                )
        return "".join(
    ...
    def include_doc_string(
        ...
        if doc_string:
            try:
                ...
                self._last_doc_string = (
                    start,
                    end,
                )
            except Exception:
                LOG.debug(
                    "Error parsing doc string",
                    exc_info=True,
                )
                LOG.debug(
                    doc_string
...
class DocumentStructure(
    ...
    def flush_structure(
        ...
        for (
            ...
            match = DOCUMENTATION_LINK_REGEX.search(
                value.decode()
            )
            docs_link = (
                f"{match.group(0)}\n\n".encode()
        ...
        section_config = SECTION_LINE_LIMIT_CONFIG.get(
            self.name
        )
        aws_docs_link = (
            docs_link.decode()
    ...
    def write_to_file(
        ...
        sub_resource_file_path = os.path.join(
            full_path,
            f"{file_name}.rst",
        )
        with open(
            sub_resource_file_path,
            "wb",
        ) as f:
            f.write(
