...
def get_official_service_name(
    ...
    short_name = service_model.metadata.get(
        "serviceAbbreviation",
        "",
    )
...
_ESCAPE_CONTROLS_RE = re.compile(
    "|".join(
        map(
            re.escape,
            _CONTROLS,
...
_CONTROLS_MATCH_HANDLER = lambda match: _CONTROLS[
    match.group(
        0
    )
]
...
def escape_controls(
    ...
    return _ESCAPE_CONTROLS_RE.sub(
