...
def set_socket_timeout(
    ...
    """
    http_response._fp.fp.raw._sock.settimeout(
        timeout
    )
...
def ensure_bytes(
    ...
    if isinstance(
        ...
        return s
    raise ValueError(
        f"Expected str or bytes, received {type(s)}."
    )
...
def _windows_shell_split(
    ...
    * Backslash is interpreted literally unless it is part of a chain that
    ...
    if is_quoted:
        raise ValueError(
            f"No closing quotation in string: {s}"
        )
