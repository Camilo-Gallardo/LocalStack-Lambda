...
class RequestSigner:
    ...
    * before-sign: Allows mutating the request before signing.
    ...
    def __init__(
        ...
        self._event_emitter = weakref.proxy(
            event_emitter
        )
    ...
    def sign(
        ...
        if (
            ...
            if (
                not explicit_region_name
                and signing_context.get(
                    "region"
                )
            ):
                kwargs[
    ...
    def _choose_signer(
        ...
        suffix = signing_type_suffix_map.get(
            signing_type,
            "",
        )
        ...
        signing_name = signing.get(
            "signing_name",
            self._signing_name,
        )
        region_name = signing.get(
            "region",
            self._region_name,
        )
        if (
            signature_version
            is not botocore.UNSIGNED
            and not signature_version.endswith(
                suffix
            )
        ):
        ...
        (
            handler,
            response,
        ) = self._event_emitter.emit_until_response(
    ...
    def get_auth_instance(
        ...
        **kwargs,
        ...
        cls = botocore.auth.AUTH_TYPE_MAPS.get(
            signature_version
        )
        if (
            cls
        ...
        if (
            ...
            if (
                ...
                frozen_token = (
                    self._auth_token.get_frozen_token()
                )
            auth = cls(
                frozen_token
            )
...
def add_generate_db_auth_token(
    class_attributes,
    **kwargs,
):
    class_attributes[
...
def add_generate_presigned_url(
    class_attributes,
    **kwargs,
):
    class_attributes[
...
def generate_presigned_url(
    ...
    context = {
        ...
        "use_global_endpoint": _should_use_global_endpoint(
            self
        ),
    }
    ...
    try:
        operation_name = self._PY_TO_OP_NAME[
            client_method
        ]
    except KeyError:
        raise UnknownClientMethodError(
    ...
    bucket_is_arn = ArnParser.is_arn(
        params.get(
            "Bucket",
            "",
        )
...
def add_generate_presigned_post(
    class_attributes,
    **kwargs,
):
    class_attributes[
...
def generate_presigned_post(
    ...
    post_presigner = S3PostPresigner(
        self._request_signer
    )
...
def _should_use_global_endpoint(
    client,
):
    if (
    ...
    if s3_config:
        ...
        if (
            ...
            == "regional"
            and client.meta.config.region_name
            == "us-east-1"
        ):
            return False
    return True
