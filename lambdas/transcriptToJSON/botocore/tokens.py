...
def _sso_json_dumps(
    ...
    return json.dumps(
        obj,
        default=_serialize_utc_timestamp,
    )
...
class SSOTokenProvider:
    ...
    def __init__(
        ...
        self._cache = cache
        self._token_loader = SSOTokenLoader(
            cache=self._cache
        )
        self._profile_name = (
            profile_name
            or self._session.get_config_variable(
                "profile"
            )
            or "default"
        )
    ...
    def _load_sso_config(
        ...
        sso_session_name = profile_config[
            "sso_session"
        ]
        sso_config = sso_sessions.get(
            sso_session_name,
            None,
        )
    ...
    def _attempt_create_token(
        ...
        new_token = {
            "startUrl": self._sso_config[
                "sso_start_url"
            ],
            "region": self._sso_config[
                "sso_region"
            ],
    ...
    def _refresh_access_token(
        ...
        if (
            ...
            logger.info(
                f"SSO token registration expired at {expiry}"
            )
            return None

        try:
            return self._attempt_create_token(
                token
            )
        except ClientError:
            logger.warning(
    ...
    def _refresher(
        ...
        start_url = self._sso_config[
            "sso_start_url"
        ]
        session_name = self._sso_config[
            "session_name"
        ]
        logger.info(
            f"Loading cached SSO token for {session_name}"
        )
        token_dict = self._token_loader(
            start_url,
        ...
        logger.debug(
            f"Cached SSO token expires at {expiration}"
        )
        ...
        if (
            ...
            new_token_dict = self._refresh_access_token(
                token_dict
            )
            if (
                new_token_dict
