...
def create_serializer(
    ...
    if include_validation:
        ...
        serializer = validate.ParamValidationDecorator(
            validator,
            serializer,
        )
    return serializer
...
class Serializer:
    ...
    def _timestamp_rfc822(
        ...
        if isinstance(
            ...
            value = self._timestamp_unixtimestamp(
                value
            )
        return formatdate(
            value,
    ...
    def _convert_timestamp_to_str(
        ...
        datetime_obj = parse_to_aware_datetime(
            value
        )
        converter = getattr(
            self,
            f"_timestamp_{timestamp_format}",
        )
    ...
    def _get_serialized_name(
        ...
        return shape.serialization.get(
            "name",
            default_name,
        )
    ...
    def _expand_host_prefix(
        ...
        return host_prefix_expression.format(
            **format_kwargs
        )
...
class QuerySerializer(
    ...
    def serialize_to_request(
        ...
        host_prefix = self._expand_host_prefix(
            parameters,
            operation_model,
        )
        if (
    ...
    def _serialize_type_structure(
        ...
        for (
            ...
            member_prefix = self._get_serialized_name(
                member_shape,
                key,
            )
            if prefix:
    ...
    def _serialize_type_list(
        ...
        if (
            ...
            return
        if self._is_shape_flattened(
            shape
        ):
            list_prefix = prefix
            if shape.member.serialization.get(
                "name"
            ):
            ...
            list_name = shape.member.serialization.get(
                "name",
                "member",
            )
    ...
    def _serialize_type_map(
        ...
        key_suffix = self._get_serialized_name(
            key_shape,
            default_name="key",
        )
        value_suffix = self._get_serialized_name(
            value_shape,
            "value",
        )
        for (
            ...
            key_prefix = template.format(
                i=i,
                suffix=key_suffix,
            )
            value_prefix = template.format(
                i=i,
                suffix=value_suffix,
            )
            self._serialize(
...
class JSONSerializer(
    ...
    def serialize_to_request(
        ...
        json_version = operation_model.metadata[
            "jsonVersion"
        ]
        serialized = (
            self._create_default_request()
        ...
        if (
            ...
            for (
                ...
                if (
                    ...
                    member_key = member_shape.serialization[
                        "name"
                    ]
                self._serialize(
                    serialized,
...
class BaseRestSerializer(
    ...
    def _render_uri_template(
        ...
        for template_param in re.findall(
            r"{(.*?)}",
            uri_template,
        ):
    ...
    def _serialize_payload(
        ...
        payload_member = shape.serialization.get(
            "payload"
        )
        if self._has_streaming_payload(
            payload_member,
            shape_members,
        ):
            # If it's streaming, then the body is just the
            # value of the payload.
            body_payload = parameters.get(
                payload_member,
                b"",
            )
            body_payload = self._encode_payload(
                body_payload
            )
            serialized[
                "body"
        ...
        elif self._requires_empty_body(
            shape
        ):
            serialized[
                "body"
    ...
    def _partition_parameters(
        ...
        member = shape_members[
            param_name
        ]
        location = member.serialization.get(
            "location"
        )
        key_name = member.serialization.get(
            "name",
            param_name,
        )
        if (
            ...
            if isinstance(
                ...
                timestamp_format = member.serialization.get(
                    "timestampFormat",
                    self.QUERY_STRING_TIMESTAMP_FORMAT,
                )
            ...
            if (
                ...
                return
            value = self._convert_header_value(
                shape,
                param_value,
            )
            partitioned[
    ...
    def _convert_header_value(
        ...
        elif is_json_value_header(
            shape
        ):
            # Serialize with no spaces after separators to save space in
            # the header.
            return self._get_base64(
                json.dumps(
                    value,
                    separators=(
                        ",",
...
class RestXMLSerializer(
    ...
    def _serialize_type_structure(
        ...
        structure_node = ElementTree.SubElement(
            xmlnode,
            name,
        )
        ...
        if (
            ...
            namespace_metadata = shape.serialization[
                "xmlNamespace"
            ]
            attribute_name = "xmlns"
            if namespace_metadata.get(
                "prefix"
            ):
                attribute_name += (
                    ":%s"
        ...
        for (
            ...
            member_name = member_shape.serialization.get(
                "name",
                key,
            )
            ...
            if member_shape.serialization.get(
                ...
                xml_attribute_name = member_shape.serialization[
                    "name"
                ]
                structure_node.attrib[
                    xml_attribute_name
    ...
    def _serialize_type_list(
        ...
        if shape.serialization.get(
            ...
            element_name = member_shape.serialization.get(
                "name",
                "member",
            )
            list_node = ElementTree.SubElement(
                xmlnode,
                name,
            )
        for item in params:
    ...
    def _serialize_type_map(
        ...
        for (
            ...
            entry_node = ElementTree.SubElement(
                node,
                "entry",
            )
            key_name = self._get_serialized_name(
                shape.key,
                default_name="key",
            )
