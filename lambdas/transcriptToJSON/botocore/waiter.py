...
class WaiterModel:
    ...
    def get_waiter(
        ...
        try:
            single_waiter_config = self._waiter_config[
                waiter_name
            ]
        except KeyError:
            raise ValueError(
...
class SingleWaiterConfig:
    ...
    def __init__(
        ...
        self.description = single_waiter_config.get(
            "description",
            "",
        )
        # Per the spec, these three fields are required.
        self.operation = single_waiter_config[
            "operation"
        ]
        self.delay = single_waiter_config[
            "delay"
        ]
        self.max_attempts = single_waiter_config[
            "maxAttempts"
        ]
    ...
    def acceptors(
        ...
        for acceptor_config in self._config[
            "acceptors"
        ]:
...
class AcceptorConfig:
    def __init__(
        ...
        self.argument = config.get(
            "argument"
        )
        self.matcher_func = (
            self._create_matcher_func()
        )
    ...
    def explanation(
        ...
        if (
            ...
            return (
                "Matched expected HTTP status code: %s"
                % self.expected
            )
        elif (
            self.matcher
            ...
            return (
                "Matched expected service error code: %s"
                % self.expected
            )
        else:
            return (
                'No explanation for unknown waiter type: "%s"'
                % self.matcher
            )

    def _create_matcher_func(
        self,
    ):
        ...
        if (
            ...
            return (
                self._create_error_matcher()
            )
        else:
            raise WaiterConfigError(
                error_msg="Unknown acceptor: %s"
                % self.matcher
            )
    ...
    def _create_path_matcher(
        ...
        def acceptor_matches(
            response,
        ):
            if is_valid_waiter_error(
                response
            ):
                return
            return (
...
class Waiter:
    ...
    def wait(
        ...
        **kwargs
        ...
        while True:
            response = self._operation_method(
                **kwargs
            )
            num_attempts += 1
            for acceptor in acceptors:
                if acceptor.matcher_func(
                    response
                ):
            ...
            if (
                ...
                logger.debug(
                    "Waiting complete, waiter matched the "
                    "success state."
                )
                return
            if (
                ...
                reason = (
                    "Waiter encountered a terminal failure state: %s"
                    % (
