...
class EndpointResolver(
    ...
    def __init__(
        ...
        if (
            "partitions"
            not in endpoint_data
        ):
            raise ValueError(
                'Missing "partitions" in endpoint data'
            )
        self._endpoint_data = endpoint_data
        self.uses_builtin_data = uses_builtin_data

    def get_service_endpoints_data(
        self,
        service_name,
        partition_name="aws",
    ):
        for partition in self._endpoint_data[
            "partitions"
        ]:
            if (
                partition[
    ...
    def _endpoint_for_partition(
        ...
        if (
            ...
            partition_endpoint = service_data.get(
                "partitionEndpoint"
            )
            is_regionalized = service_data.get(
                "isRegionalized",
                True,
            )
            if (
                partition_endpoint
                and not is_regionalized
            ):
                LOG.debug(
    ...
    def _retrieve_variant_data(
        self,
        endpoint_data,
        tags,
    ):
        variants = endpoint_data.get(
            "variants",
            [],
        )
    ...
    def _resolve(
        ...
        endpoint_data = service_data.get(
            "endpoints",
            {},
        ).get(
            endpoint_name,
        ...
        service_defaults = service_data.get(
            "defaults",
            {},
        )
        partition_defaults = partition.get(
            "defaults",
            {},
        )
        tags = self._create_tag_list(
            use_dualstack_endpoint,
            use_fips_endpoint,
        )
...
class EndpointResolverBuiltins(
    str,
    Enum,
):
...
class EndpointRulesetResolver:
    ...
    def __init__(
        ...
        self._param_definitions = (
            self._provider.ruleset.parameters
        )
    ...
    def construct_endpoint(
        ...
        LOG.debug(
            "Calling endpoint provider with parameters: %s"
            % provider_params
        )
        try:
        ...
        if (
            not self._use_ssl
            and provider_result.url.startswith(
                "https://"
            )
        ):
        ...
        provider_result = provider_result._replace(
            headers={
                ...
                for key, val in provider_result.headers.items()
            }
        )
    ...
    def _resolve_param_from_context(
        ...
        if (
            ...
            return dynamic
        return self._resolve_param_as_client_context_param(
            param_name
        )
    ...
    def _resolve_param_as_static_context_param(
        ...
        static_ctx_params = self._get_static_context_params(
            operation_model
        )
        return static_ctx_params.get(
            param_name
        )
    ...
    def _resolve_param_as_dynamic_context_param(
        ...
        dynamic_ctx_params = self._get_dynamic_context_params(
            operation_model
        )
        if (
            param_name
            in dynamic_ctx_params
        ):
            member_name = dynamic_ctx_params[
                param_name
            ]
            return call_args.get(
                member_name
            )
    ...
    def _resolve_param_as_client_context_param(
        ...
        client_ctx_params = (
            self._get_client_context_params()
        )
        if (
            param_name
            in client_ctx_params
        ):
            client_ctx_varname = client_ctx_params[
                param_name
            ]
            return self._client_context.get(
                client_ctx_varname
            )
    ...
    def _resolve_param_as_builtin(
        ...
        if (
            builtin_name
            not in EndpointResolverBuiltins.__members__.values()
        ):
            raise UnknownEndpointResolutionBuiltInName(
                name=builtin_name
            )
        return builtins.get(
            builtin_name
    ...
    @instance_cache
    def _get_static_context_params(
        self,
        operation_model,
    ):
    ...
    @instance_cache
    def _get_dynamic_context_params(
        self,
        operation_model,
    ):
    ...
    def auth_schemes_to_signing_ctx(
        ...
        if (
            ...
            raise TypeError(
                "auth_schemes must be a non-empty list."
            )
    ...
    def _strip_sig_prefix(
        ...
        return (
            ...
            if auth_name.startswith(
                "sig"
            )
            else auth_name
        )

    def _does_botocore_authname_match_ruleset_authname(
        self,
        botoname,
        rsname,
    ):
        ...
        * s3-presign-post, sigv4
        """
        rsname = self._strip_sig_prefix(
            rsname
        )
        botoname = botoname.split(
            "-"
        )[
            0
        ]
    ...
    def ruleset_error_to_botocore_exception(
        ...
        """
        msg = ruleset_exception.kwargs.get(
            "msg"
        )
        if (
            msg
        ...
        if (
            ...
            if (
                ...
                return UnsupportedS3ConfigurationError(
                    msg=msg
                )
            if (
                msg.startswith(
                    "S3 Outposts does not support"
                )
                or msg.startswith(
                    "S3 MRAP does not support"
                )
                or msg.startswith(
                    "S3 Object Lambda does not support"
                )
                or msg.startswith(
                    "Access Points do not support"
                )
                or msg.startswith(
                ...
                return UnsupportedS3AccesspointConfigurationError(
                    msg=msg
                )
        ...
        if (
            ...
            if msg.startswith(
                ...
                return UnsupportedS3ControlArnError(
                    arn=arn,
                    msg=msg,
                )
            if msg.startswith(
                ...
                return UnsupportedS3ControlConfigurationError(
                    msg=msg
                )
            if (
                msg
        ...
        if (
            ...
            if msg.startswith(
                ...
                return InvalidEndpointConfigurationError(
                    msg=msg
                )
            if (
                msg
