...
def create_credential_resolver(
    ...
    disable_env_vars = (
        session.instance_variables().get(
            "profile"
        )
        is not None
    )
...
class ProfileProviderBuilder:
    ...
    def providers(
        ...
        return [
            ...
            self._create_sso_provider(
                profile_name
            ),
            self._create_shared_credential_provider(
                profile_name
            ),
            self._create_process_provider(
                profile_name
            ),
            self._create_config_provider(
                profile_name
            ),
        ]

    def _create_process_provider(
        self,
        profile_name,
    ):
    ...
    def _create_shared_credential_provider(
        self,
        profile_name,
    ):
        credential_file = self._session.get_config_variable(
            "credentials_file"
        )
    ...
    def _create_config_provider(
        self,
        profile_name,
    ):
        config_file = self._session.get_config_variable(
            "config_file"
        )
    ...
    def _create_web_identity_provider(
        self,
        profile_name,
        disable_env_vars,
    ):
...
def create_assume_role_refresher(
    ...
    def refresh():
        ...
        return {
            ...
            "expiry_time": _serialize_if_needed(
                credentials[
                    "Expiration"
                ]
            ),
...
class Credentials:
    ...
    def _normalize(
        ...
        self.access_key = botocore.compat.ensure_unicode(
            self.access_key
        )
        self.secret_key = botocore.compat.ensure_unicode(
            self.secret_key
        )
...
class RefreshableCredentials(
    ...
    def _normalize(
        ...
        self._access_key = botocore.compat.ensure_unicode(
            self._access_key
        )
        self._secret_key = botocore.compat.ensure_unicode(
            self._secret_key
        )
    ...
    def create_from_metadata(
        ...
        instance = cls(
            ...
            expiry_time=cls._expiry_datetime(
                metadata[
                    "expiry_time"
                ]
            ),
...
class CachedCredentialFetcher:
    ...
    def __init__(
        ...
        if (
            ...
            expiry_window_seconds = (
                self.DEFAULT_EXPIRY_WINDOW_SECONDS
            )
    ...
    def _get_cached_credentials(
        ...
        if (
            ...
            logger.debug(
                "Credentials for role retrieved from cache."
            )

        creds = response[
            "Credentials"
        ]
        expiration = _serialize_if_needed(
            creds[
                "Expiration"
            ],
            iso=True,
...
class BaseAssumeRoleCredentialFetcher(
    ...
    def __init__(
        ...
        self._role_session_name = self._assume_kwargs.get(
            "RoleSessionName"
        )
        self._using_default_session_name = False
        if (
            not self._role_session_name
        ):
    ...
    def _generate_assume_role_name(
        self,
    ):
    ...
    def _create_cache_key(
        ...
        # in the hash.
        if (
            self._using_default_session_name
        ):
            del args[
                "RoleSessionName"
            ]
...
class AssumeRoleCredentialFetcher(
    ...
    def _assume_role_kwargs(
        self,
    ):
        """Get the arguments for assume role based on current configuration."""
        assume_role_kwargs = deepcopy(
            self._assume_kwargs
        )

        mfa_serial = assume_role_kwargs.get(
            "SerialNumber"
        )
        ...
        if (
            ...
            prompt = (
                "Enter MFA code for %s: "
                % mfa_serial
            )
            token_code = self._mfa_prompter(
                prompt
            )
            assume_role_kwargs[
                "TokenCode"
        ...
        duration_seconds = assume_role_kwargs.get(
            "DurationSeconds"
        )
...
class AssumeRoleWithWebIdentityCredentialFetcher(
    ...
    def _assume_role_kwargs(
        ...
        identity_token = (
            self._web_identity_token_loader()
        )
        assume_role_kwargs[
            "WebIdentityToken"
        ] = identity_token
...
class CredentialProvider:
    ...
    def _extract_creds_from_mapping(
        self,
        mapping,
        *key_names,
    ):
...
class ProcessProvider(
    ...
    def load(
        ...
        creds_dict = self._retrieve_credentials_using(
            credential_process
        )
        if (
            creds_dict.get(
        ...
        return Credentials(
            access_key=creds_dict[
                "access_key"
            ],
            secret_key=creds_dict[
                "secret_key"
            ],
            token=creds_dict.get(
                "token"
            ),
            method=self.METHOD,
        )

    def _retrieve_credentials_using(
        self,
        credential_process,
    ):
        # We're not using shell=True, so we need to pass the
        # command and all arguments as a list.
        process_list = compat_shell_split(
            credential_process
        )
        p = self._popen(
            process_list,
        ...
        parsed = botocore.compat.json.loads(
            stdout.decode(
                "utf-8"
            )
        )
...
class InstanceMetadataProvider(
    ...
    def load(
        ...
        if (
            ...
            return None
        logger.info(
            "Found credentials from IAM Role: %s",
            metadata[
                "role_name"
            ],
        )
...
class EnvProvider(
    ...
    def load(
        ...
        if access_key:
            ...
            fetcher = (
                self._create_credentials_fetcher()
            )
            credentials = fetcher(
    ...
    def _create_credentials_fetcher(
        self,
    ):
        mapping = (
            self._mapping
    ...
    def load(
        ...
        if (
            ...
            if (
                ...
                logger.info(
                    "Found credentials in AWS_CREDENTIAL_FILE."
                )
                access_key = creds[
                    self.ACCESS_KEY
                ]
...
class SharedCredentialProvider(
    ...
    def load(
        ...
        try:
            available_creds = self._ini_parser(
        ...
        if (
            ...
            if (
                ...
                (
                    access_key,
                    secret_key,
                ) = self._extract_creds_from_mapping(
                    config,
                    self.ACCESS_KEY,
                    self.SECRET_KEY,
                )
...
class ConfigProvider(
    ...
    def __init__(
        ...
        if (
            ...
            config_parser = (
                botocore.configloader.load_config
            )
    ...
    def load(
        ...
        """
        try:
            full_config = self._config_parser(
...
class BotoProvider(
    ...
    def load(
        ...
        if (
            ...
            potential_locations = (
                self.DEFAULT_CONFIG_FILENAMES
            )
        for filename in potential_locations:
            try:
                config = self._ini_parser(
                    filename
                )
            except ConfigNotFound:
            ...
            if (
                ...
                if (
                    ...
                    logger.info(
                        "Found credentials in boto config file: %s",
                        filename,
                    )
                    (
                        access_key,
...
class AssumeRoleProvider(
    ...
    def load(
        ...
        profiles = self._loaded_config.get(
            "profiles",
            {},
        )
    ...
    def _has_assume_role_config_vars(
        self,
        profile,
    ):
        return (
            self.ROLE_CONFIG_VAR
            in profile
            and
            ...
            self.WEB_IDENTITY_TOKE_FILE_VAR
            not in profile
        )

    def _load_creds_via_assume_role(
        self,
        profile_name,
    ):
        ...
        role_session_name = role_config.get(
            "role_session_name"
        )
        if (
            role_session_name
        ...
        duration_seconds = role_config.get(
            "duration_seconds"
        )
        if (
            duration_seconds
    ...
    def _get_role_config(
        ...
        mfa_serial = profile.get(
            "mfa_serial"
        )
        external_id = profile.get(
            "external_id"
        )
        role_session_name = profile.get(
            "role_session_name"
        )
    ...
    def _validate_credential_source(
        ...
        if not self._credential_sourcer.is_supported(
            credential_source
        ):
            raise InvalidConfigError(
                error_msg=(
    ...
    def _source_profile_has_credentials(
        self,
        profile,
    ):
        return any(
            [
                self._has_static_credentials(
                    profile
                ),
                self._has_assume_role_config_vars(
                    profile
                ),
            ]
        )
    ...
    def _validate_source_profile(
        ...
        # precedence.
        if not self._has_static_credentials(
            source_profile
        ):
            raise InfiniteLoopConfigError(
                source_profile=source_profile_name,
    ...
    def _resolve_source_credentials(
        ...
        credential_source = role_config.get(
            "credential_source"
        )
        if (
            credential_source
        ...
        source_profile = role_config[
            "source_profile"
        ]
        self._visited_profiles.append(
            source_profile
        )
        return self._resolve_credentials_from_profile(
            source_profile
        )

    def _resolve_credentials_from_profile(
        self,
        profile_name,
    ):
        profiles = self._loaded_config.get(
        ...
        if (
            ...
            return self._resolve_static_credentials_from_profile(
                profile
            )
        elif self._has_static_credentials(
            profile
        ) or not self._has_assume_role_config_vars(
            profile
        ):
    ...
    def _resolve_static_credentials_from_profile(
        self,
        profile,
    ):
        try:
...
class AssumeRoleWithWebIdentityProvider(
    ...
    def _get_profile_config(
        ...
        if (
            ...
            profiles = loaded_config.get(
                "profiles",
                {},
            )
            self._profile_config = profiles.get(
                self._profile_name,
                {},
            )
        return self._profile_config.get(
            key
        )
    ...
    def _get_env_config(
        ...
        if (
            self._disable_env_vars
        ):
            return None
        env_key = self._CONFIG_TO_ENV_VAR.get(
            key
        )
        if (
            env_key
    ...
    def _get_config(
        ...
        env_value = self._get_env_config(
            key
        )
        if (
            env_value
            ...
            return env_value
        return self._get_profile_config(
            key
        )
    ...
    def _assume_role_with_web_identity(
        ...
        role_session_name = self._get_config(
            "role_session_name"
        )
        if (
            role_session_name
...
class CanonicalNameCredentialSourcer:
    ...
    def _get_provider(
        ...
        :raises UnknownCredentialError: Raised if no
            ...
            is found.
        """
        provider = self._get_provider_by_canonical_name(
            canonical_name
        )
        ...
        if canonical_name.lower() in [
            "sharedconfig",
            "sharedcredentials",
        ]:
            assume_role_provider = self._get_provider_by_method(
                "assume-role"
            )
            if (
                assume_role_provider
    ...
    def _get_provider_by_canonical_name(
        self,
        canonical_name,
    ):
        """Return a credential provider by its canonical name.
...
class ContainerProvider(
    ...
    def load(
        ...
        if (
            self.ENV_VAR
            in self._environ
            or self.ENV_VAR_FULL
            in self._environ
        ):
            return (
                self._retrieve_or_fail()
            )

    def _retrieve_or_fail(
        self,
    ):
        if (
            self._provided_relative_uri()
        ):
            full_uri = self._fetcher.full_url(
                self._environ[
                    self.ENV_VAR
                ]
            )
        else:
            full_uri = self._environ[
                self.ENV_VAR_FULL
            ]
        headers = (
            self._build_headers()
    ...
    def _build_headers(
        ...
        auth_token = self._environ.get(
            self.ENV_VAR_AUTH_TOKEN
        )
        if (
            auth_token
            is not None
    ...
    def _create_fetcher(
        ...
        def fetch_creds():
            try:
                ...
                logger.debug(
                    "Error retrieving container metadata: %s",
                    e,
                    exc_info=True,
                )
                raise CredentialRetrievalError(
...
class CredentialResolver:
    ...
    def insert_after(
        ...
        :type cred_instance: A subclass of ``Credentials``
        """
        offset = self._get_provider_offset(
            name
        )
        self.providers.insert(
            offset
    ...
    def remove(
        ...
        offset = available_methods.index(
            name
        )
        self.providers.pop(
            offset
...
class SSOCredentialFetcher(
    ...
    def _get_credentials(
        ...
        if (
            ...
            initial_token_data = (
                self._token_provider.load_token()
            )
            token = (
                initial_token_data.get_frozen_token().token
            )
        else:
...
class SSOProvider(
    ...
    def _load_sso_config(
        ...
        sso_sessions = loaded_config.get(
            "sso_sessions",
            {},
        )
        ...
        if all(
            c
            not in profile_config
            for c in self._PROFILE_REQUIRED_CONFIG_VARS
        ):
            return None
        ...
        (
            resolved_config,
            extra_reqs,
        ) = self._resolve_sso_session_reference(
            profile_config,
            sso_sessions,
        )
        ...
        all_required_configs = (
            self._ALL_REQUIRED_CONFIG_VARS
            + extra_reqs
        )
    ...
    def _resolve_sso_session_reference(
        self,
        profile_config,
        sso_sessions,
    ):
        sso_session_name = profile_config.get(
            "sso_session"
        )
        if (
            sso_session_name
    ...
    def load(
        ...
        fetcher_kwargs = {
            "start_url": sso_config[
                "sso_start_url"
            ],
            "sso_region": sso_config[
                "sso_region"
            ],
            "role_name": sso_config[
                "sso_role_name"
            ],
            "account_id": sso_config[
                "sso_account_id"
            ],
            "client_creator": self._client_creator,
            "token_loader": SSOTokenLoader(
