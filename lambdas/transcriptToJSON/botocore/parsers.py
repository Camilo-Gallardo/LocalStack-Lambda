...
def create_parser(
    ...
    return ResponseParserFactory().create_parser(
        protocol
    )
...
class ResponseParser:
    ...
    def parse(
        ...
        if (
            ...
            if self._is_generic_error_response(
                response
            ):
                parsed = self._do_generic_error_parse(
                    response
                )
            elif self._is_modeled_error_shape(
                shape
            ):
                parsed = self._do_modeled_error_parse(
                    response,
                    shape,
                )
        ...
        if isinstance(
            ...
            self._add_checksum_response_metadata(
                response,
                response_metadata,
            )
        return parsed

    def _add_checksum_response_metadata(
        self,
        response,
        response_metadata,
    ):
        checksum_context = response.get(
            "context",
            {},
        ).get(
            "checksum",
        ...
        algorithm = checksum_context.get(
            "response_algorithm"
        )
        if algorithm:
            response_metadata[
    ...
    def _is_modeled_error_shape(
        ...
        return (
            shape
            is not None
            and shape.metadata.get(
                "exception",
                False,
            )
        )

    def _is_generic_error_response(
        self,
        response,
    ):
    ...
    def _do_error_parse(
        ...
        raise NotImplementedError(
            f"{self.__class__.__name__}._do_error_parse"
        )

    def _do_modeled_error_parse(
        self,
        response,
        shape,
        parsed,
    ...
    def _has_unknown_tagged_union_member(
        self,
        shape,
        value,
    ):
        if (
            shape.is_tagged_union
        ):
            cleaned_value = (
                value.copy()
            )
            ...
            tag = self._get_first_key(
                cleaned_value
            )
            if (
                tag
                not in shape.members
            ):
                msg = (
    ...
    def _handle_unknown_tagged_union_member(
        self,
        tag,
    ):
        return {
...
class BaseXMLResponseParser(
    ...
    def _handle_map(
        ...
        key_location_name = (
            key_shape.serialization.get(
                "name"
            )
            or "key"
        )
        value_location_name = (
            value_shape.serialization.get(
                "name"
            )
            or "value"
        )
        if shape.serialization.get(
            "flattened"
        ) and not isinstance(
            node,
            list,
    ...
    def _handle_structure(
        ...
        if shape.metadata.get(
            "exception",
            False,
        ):
            node = self._get_error_root(
                node
            )
        xml_dict = self._build_name_to_xml_node(
            node
        )
        if self._has_unknown_tagged_union_member(
            shape,
            xml_dict,
        ):
            tag = self._get_first_key(
            ...
            return self._handle_unknown_tagged_union_member(
                tag
            )
        for member_name in members:
            member_shape = members[
                member_name
            ]
            if (
                "location"
                in member_shape.serialization
                or member_shape.serialization.get(
                    "eventheader"
                )
            ):
                ...
                continue
            xml_name = self._member_key_name(
                member_shape,
                member_name,
            )
            member_node = xml_dict.get(
                xml_name
            )
            if (
                member_node
            ...
            elif member_shape.serialization.get(
                "xmlAttribute"
            ):
                attribs = (
                    {}
                )
                location_name = member_shape.serialization[
                    "name"
                ]
                for (
                    key,
    ...
    def _build_name_to_xml_node(
        ...
        if isinstance(
            ...
            return self._build_name_to_xml_node(
                parent_node[
                    0
                ]
            )
    ...
    def _replace_nodes(
        ...
        for (
            ...
            if list(
                ...
                sub_dict = self._build_name_to_xml_node(
                    value
                )
                parsed[
                    key
...
class QueryParser(
    ...
    def _do_error_parse(
        ...
        xml_contents = response[
            "body"
        ]
        root = self._parse_xml_string_to_dom(
            xml_contents
        )
        parsed = self._build_name_to_xml_node(
            root
        )
        self._replace_nodes(
            parsed
    ...
    def _do_modeled_error_parse(
        ...
        return self._parse_body_as_xml(
            response,
            shape,
    ...
    def _find_result_wrapped_shape(
        self,
        element_name,
        xml_root_node,
    ):
        mapping = self._build_name_to_xml_node(
            xml_root_node
        )
        return mapping[
            element_name
    ...
    def _inject_response_metadata(
        ...
        if (
            ...
            sub_mapping = self._build_name_to_xml_node(
                child_node
            )
            for (
                key,
...
class EC2QueryParser(
    ...
    def _do_error_parse(
        ...
        # not RequestId
        original = super()._do_error_parse(
            response,
            shape,
        )
        if (
...
class BaseJSONParser(
    ...
    def _handle_structure(
        ...
        if (
            ...
            for member_name in member_shapes:
                ...
                json_name = member_shape.serialization.get(
                    "name",
                    member_name,
                )
                raw_value = value.get(
    ...
    def _handle_map(
        ...
        for (
            ...
            actual_key = self._parse_shape(
                key_shape,
                key,
            )
            actual_value = self._parse_shape(
                value_shape,
                value,
            )
            parsed[
    ...
    def _do_query_compatible_error_parse(
        self,
        code,
        headers,
        error,
        ...
        if (
            len(
                query_error_components
            )
            == 2
            and query_error_components[
                0
            ]
        ):
            error[
...
class BaseEventStreamParser(
    ...
    def _do_error_parse(
        ...
        exception_type = response[
            "headers"
        ].get(
            ":exception-type"
        )
        exception_shape = shape.members.get(
            exception_type
        )
        if (
            exception_shape
            ...
            original_parsed = self._initial_body_parse(
                response[
                    "body"
                ]
            )
            body = self._parse_shape(
                exception_shape,
                original_parsed,
            )
            error = {
    ...
    def _parse_payload(
        ...
        if shape.serialization.get(
            ...
            for name in member_shapes:
                ...
                if member_shape.serialization.get(
                    "eventpayload"
                ):
                    body = response[
                        "body"
                    ...
                    if (
                        ...
                        raw_parse = self._initial_body_parse(
                            body
                        )
...
class EventStreamJSONParser(
    ...
    def _initial_body_parse(
        ...
        return self._parse_body_as_json(
            body_contents
        )


class EventStreamXMLParser(
    BaseEventStreamParser,
    BaseXMLResponseParser,
):
    def _initial_body_parse(
...
class JSONParser(
    ...
    def _handle_event_stream(
        ...
        event_stream_shape = shape.members[
            event_name
        ]
    ...
    def _handle_json_body(
        ...
        parsed_json = self._parse_body_as_json(
            raw_body
        )
...
class BaseRestParser(
    ...
    def _do_parse(
        ...
        final_parsed[
            "ResponseMetadata"
        ] = self._populate_response_metadata(
            response
        )
    ...
    def _parse_payload(
        ...
        if (
            ...
            payload_member_name = shape.serialization[
                "payload"
            ]
            body_shape = member_shapes[
                payload_member_name
            ]
            if body_shape.serialization.get(
                "eventstream"
            ):
                body = self._create_event_stream(
                    response,
                    body_shape,
                )
                final_parsed[
    ...
    def _parse_non_payload_attrs(
        ...
        for name in member_shapes:
            ...
            location = member_shape.serialization.get(
                "location"
            )
            if (
                location
    ...
    def _parse_header_map(
        ...
        prefix = shape.serialization.get(
            "name",
            "",
        ).lower()
    ...
    def _handle_list(
        ...
        location = shape.serialization.get(
            "location"
        )
        if (
            location
...
class RestJSONParser(
    ...
    def _do_error_parse(
        ...
        error = super()._do_error_parse(
            response,
            shape,
        )
        self._inject_error_code(
            error,
            response,
        )
        return error

    def _inject_error_code(
        self,
        error,
        response,
    ):
...
class RestXMLParser(
    ...
    def _do_error_parse(
        ...
        if response[
            ...
            try:
                return self._parse_error_from_body(
                    response
                )
        ...
        return self._parse_error_from_http_status(
            response
        )

    def _parse_error_from_http_status(
        self,
        response,
    ):
        return {
    ...
    def _parse_error_from_body(
        ...
        if (
            ...
            # response metadata.
            metadata = self._populate_response_metadata(
                response
            )
