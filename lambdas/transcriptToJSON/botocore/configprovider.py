...
def create_botocore_default_config_mapping(
    session,
):
...
def _create_config_chain_mapping(
    chain_builder,
    config_variables,
):
    mapping = (
    ...
    for (
        ...
        mapping[
            ...
            config_property_names=config[
                0
            ],
            default=config[
                2
            ],
            conversion_func=config[
                3
            ],
        )
    return mapping
...
class DefaultConfigResolver:
    def __init__(
        ...
        self._base_default_config = default_config_data[
            "base"
        ]
        self._modes = default_config_data[
            "modes"
        ]
    ...
    def _resolve_default_values_by_mode(
        self,
        mode,
    ):
        default_config = (
            self._base_default_config.copy()
        )
        modifications = self._modes.get(
            mode
        )
        ...
        for config_var in modifications:
            ...
            modification = list(
                modification_dict.keys()
            )[
                0
            ]
            modification_value = modification_dict[
                modification
            ]
            if (
                modification
    ...
    def get_default_config_values(
        ...
        if (
            mode
            not in self._resolved_default_configurations
        ):
            defaults = self._resolve_default_values_by_mode(
                mode
            )
            self._resolved_default_configurations[
                mode
            ] = defaults
        return self._resolved_default_configurations[
            mode
        ]
...
class ConfigChainFactory:
    ...
    def create_config_chain(
        ...
        if (
            ...
            providers.extend(
                self._get_scoped_config_providers(
                    config_property_names
                )
            )
        if (
    ...
    def _get_scoped_config_providers(
        self,
        config_property_names,
    ):
...
class ConfigValueStore:
    ...
    def __deepcopy__(
        ...
        for (
            ...
            config_store.set_config_variable(
                logical_name,
                override_value,
            )
...
class SmartDefaultsConfigStoreFactory:
    ...
    def resolve_auto_mode(
        ...
        if (
            ...
            if (
                self._instance_metadata_region
            ):
                current_region = (
                    self._instance_metadata_region
                )
    ...
    def _update_provider(
        ...
        original_provider = config_store.get_config_provider(
            variable
        )
        default_provider = ConstantProvider(
            value
        )
        if isinstance(
            original_provider,
            ...
            chain_provider_copy = copy.deepcopy(
                original_provider
            )
            chain_provider_copy.set_default_provider(
                default_provider
            )
    ...
    def _update_section_provider(
        ...
        section_provider_copy = copy.deepcopy(
            config_store.get_config_provider(
                section_name
            )
        )
    ...
    def _set_s3UsEast1RegionalEndpoints(
        self,
        config_store,
        value,
    ):
...
class InstanceVarProvider(
    ...
    def provide(
        ...
        instance_vars = (
            self._session.instance_variables()
        )
        value = instance_vars.get(
            self._instance_var
        )
        return value
...
class ScopedConfigProvider(
    ...
    def provide(
        ...
        scoped_config = (
            self._session.get_scoped_config()
        )
        if isinstance(
            self._config_var_name,
            tuple,
        ):
            section_config = scoped_config.get(
                self._config_var_name[
                    0
                ]
            )
            if not isinstance(
                ...
                return None
            return section_config.get(
                self._config_var_name[
                    1
                ]
            )
        return scoped_config.get(
            self._config_var_name
        )
...
class SectionConfigProvider(
    ...
    def provide(
        ...
        section_config = (
            self._scoped_config_provider.provide()
        )
        if (
            section_config
            and not isinstance(
        ...
        for (
            ...
            self._override_providers.items()
        ):
            provider_val = (
                provider.provide()
            )
    ...
    def set_default_provider(
        ...
        provider = self._override_providers.get(
            key
        )
        if isinstance(
            provider,
...
class ConstantProvider(
    ...
    def __repr__(
        ...
        return (
            "ConstantProvider(value=%s)"
            % self._value
        )


class ConfiguredEndpointProvider(
    BaseProvider
):
    ...
    def provide(
        ...
        """
        for location in (
            self._ENDPOINT_URL_LOOKUP_ORDER
        ):
            logger.debug(
    ...
    def _get_snake_case_service_id(
        self,
        client_name,
    ):
        ...
        client_name = utils.SERVICE_NAME_ALIASES.get(
            client_name,
            client_name,
        )
        ...
        return hyphenized_service_id.replace(
            "-",
            "_",
        )

    def _get_service_env_var_name(
        self,
    ):
        transformed_service_id_env = (
            self._transformed_service_id.upper()
        )
    ...
    def _get_services_config(
        ...
        if (
            "services"
            not in self._scoped_config
        ):
            return (
                {}
            )

        section_name = self._scoped_config[
            "services"
        ]
        services_section = self._full_config.get(
            "services",
            {},
        ).get(
            section_name
    ...
    def _get_endpoint_url_config_service(
        self,
    ):
        snakecase_service_id = (
            self._transformed_service_id.lower()
        )
        return (
    ...
    def _get_endpoint_url_config_global(
        self,
    ):
        return self._scoped_config.get(
            "endpoint_url"
        )

    def _get_endpoint_url_environment_service(
        self,
    ):
        return EnvironmentProvider(
    ...
    def _get_endpoint_url_environment_global(
        self,
    ):
        return EnvironmentProvider(
