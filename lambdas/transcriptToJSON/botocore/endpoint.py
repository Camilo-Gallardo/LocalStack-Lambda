...
class Endpoint:
    ...
    def create_request(
        ...
        if operation_model:
            ...
            service_id = (
                operation_model.service_model.service_id.hyphenize()
            )
    ...
    def _set_ttl(
        ...
        response_date_header = success_response[
            0
        ].headers.get(
            "Date"
        )
        has_streaming_input = retries_context.get(
            "has_streaming_input"
        )
        if (
            response_date_header
            and not has_streaming_input
        ):
            try:
                response_received_timestamp = (
                    datetime.datetime.utcnow()
    ...
    def _update_retries_context(
        ...
        retries_context = context.setdefault(
            "retries",
            {},
        )
        retries_context[
    ...
    def _do_get_response(
        ...
        try:
            ...
            http_response = first_non_none_response(
                responses
            )
            if (
                http_response
        ...
        protocol = operation_model.metadata[
            "protocol"
        ]
    ...
    def _add_modeled_error_fields(
        ...
        if (
            ...
            return
        service_model = (
            operation_model.service_model
        )
        error_shape = service_model.shape_for_error_code(
            error_code
        )
        if (
            error_shape
    ...
    def _needs_retry(
        ...
        handler_response = first_non_none_response(
            responses
        )
        if (
            handler_response
...
class EndpointCreator:
    ...
    def create_endpoint(
        ...
        if not is_valid_endpoint_url(
            endpoint_url
        ) and not is_valid_ipv6_endpoint_url(
            endpoint_url
        ):
            raise ValueError(
                "Invalid endpoint: %s"
    ...
    def _get_proxies(
        ...
        return get_environ_proxies(
            url
        )

    def _get_verify_value(
        self,
        verify,
    ):
