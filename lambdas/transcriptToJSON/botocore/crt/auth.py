...
class CrtSigV4Auth(
    ...
    _SIGNATURE_TYPE = (
        awscrt.auth.AwsSignatureType.HTTP_REQUEST_HEADERS
    )
    ...
    def add_auth(
        ...
        existing_sha256 = self._get_existing_sha256(
            request
        )
        ...
        elif self._should_sha256_sign_payload(
            request
        ):
            if existing_sha256:
        ...
        if self._should_add_content_sha256_header(
            explicit_payload
        ):
            body_header = (
            ...
            body_header = (
                awscrt.auth.AwsSignedBodyHeaderType.NONE
            )
        ...
        future = awscrt.auth.aws_sign_request(
            crt_request,
            signing_config,
        )
        future.result()
        self._apply_signing_changes(
            request,
            crt_request,
        )
    ...
    def _apply_signing_changes(
        ...
        aws_request.headers = HTTPHeaders.from_pairs(
            list(
                signed_crt_request.headers
            )
        )

    def _should_sign_header(
        self,
        name,
        **kwargs,
    ):
    ...
    def _modify_request_before_signing(
        ...
        for h in (
            self._PRESIGNED_HEADERS_BLOCKLIST
        ):
            if (
                h
                in request.headers
    ...
    def _should_add_content_sha256_header(
        self,
        explicit_payload,
    ):
...
class CrtSigV4AsymQueryAuth(
    ...
    def _apply_signing_changes(
        ...
        signed_query = urlsplit(
            signed_crt_request.path
        ).query
        p = urlsplit(
            aws_request.url
        )
...
class CrtS3SigV4AsymQueryAuth(
    CrtSigV4AsymQueryAuth
):
