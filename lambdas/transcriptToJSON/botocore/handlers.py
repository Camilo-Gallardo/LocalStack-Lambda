...
VERSION_ID_SUFFIX = re.compile(
    r"\?versionId=[^\s]+$"
)


def handle_service_name_alias(
    service_name,
    **kwargs,
):
    return SERVICE_NAME_ALIASES.get(
        service_name,
        service_name,
    )


def add_recursion_detection_header(
    params,
    **kwargs,
):
    has_lambda_name = (
...
def _looks_like_special_case_error(
    http_response,
):
    if (
...
def decode_quoted_jsondoc(
    ...
    try:
        ...
        logger.debug(
            "Error loading quoted JSON",
            exc_info=True,
        )
    return value


def json_decode_template_body(
    parsed,
    **kwargs,
):
    if (
...
def validate_bucket_name(
    ...
    **kwargs,
    ...
    if not VALID_BUCKET.search(
        bucket
    ) and not VALID_S3_ARN.search(
        bucket
    ):
        error_msg = (
...
def _sse_md5(
    params,
    sse_member_prefix="SSECustomer",
):
    if not _needs_s3_sse_customization(
        params,
        sse_member_prefix,
    ):
        return
    ...
    if isinstance(
        ...
        key_as_bytes = key_as_bytes.encode(
            "utf-8"
        )
    ...
    key_b64_encoded = base64.b64encode(
        key_as_bytes
    ).decode(
        "utf-8"
    )
...
def _needs_s3_sse_customization(
    params,
    sse_member_prefix,
):
    return (
...
def document_copy_source_form(
    ...
    **kwargs,
    ...
    if (
        ...
        parent = section.get_section(
            "structure-value"
        )
        param_line = parent.get_section(
            "CopySource"
        )
        value_portion = param_line.get_section(
            "member-value"
        )
        ...
        param_section = section.get_section(
            "CopySource"
        )
        type_section = param_section.get_section(
            "param-type"
        )
        ...
        doc_section = param_section.get_section(
            "param-documentation"
        )
...
def _quote_source_header_from_dict(
    ...
    try:
        ...
        version_id = source_dict.get(
            "VersionId"
        )
        if VALID_S3_ARN.search(
            bucket
        ):
...
def _quote_source_header(
    value,
):
    result = VERSION_ID_SUFFIX.search(
        value
    )
    if (
        result
...
def _get_cross_region_presigned_url(
    ...
    request_dict_copy = copy.deepcopy(
        request_dict
    )
    request_dict_copy[
        "body"
...
def _get_presigned_url_source_and_destination_regions(
    request_signer,
    params,
):
    # Gets the source and destination regions to be used
    destination_region = (
        request_signer._region_name
    )
    source_region = params.get(
        "SourceRegion"
    )
    return (
        source_region,
...
def document_base64_encoding(
    ...
    append = AppendParamDocumentation(
        param,
        description,
    )
    return (
        append.append_documentation
    )


def validate_ascii_metadata(
    params,
    **kwargs,
):
...
def document_glacier_tree_hash_checksum():
    doc = """
        ...
        """
    return AppendParamDocumentation(
        "checksum",
        doc,
...
def document_cloudformation_get_template_return_type(
    ...
    **kwargs,
    ...
    if (
        ...
        template_body_section = section.get_section(
            "TemplateBody"
        )
        type_section = template_body_section.get_section(
            "param-type"
        )
        type_section.clear_text()
        type_section.write(
...
def switch_host_machinelearning(
    request,
    **kwargs,
):
    switch_host_with_param(
...
def decode_list_object_versions(
    parsed,
    context,
    **kwargs,
):
...
def convert_body_to_file_like_object(
    params,
    **kwargs,
):
    if (
...
class ParameterAlias:
    ...
    def alias_parameter_in_documentation(
        ...
        **kwargs,
        ...
        if event_name.startswith(
            ...
            if (
                self._original_name
                not in section.available_sections
            ):
                return
            ...
            param_type_section = param_section.get_section(
                "param-type"
            )
            ...
            param_name_section = param_section.get_section(
                "param-name"
            )
            self._replace_content(
                param_name_section
    ...
    def _replace_content(
        ...
        content = section.getvalue().decode(
            "utf-8"
        )
...
class HeaderToHostHoister:
    ...
    def hoist(
        ...
        **kwargs,
        ...
        self._ensure_header_is_valid_host(
            header_value
        )
        original_url = params[
            "url"
        ]
        new_url = self._prepend_to_host(
            original_url,
            header_value,
        )
        params[
    ...
    def _ensure_header_is_valid_host(
        self,
        header,
    ):
        match = self._VALID_HOSTNAME.match(
            header
        )
        if (
            not match
    ...
    def _prepend_to_host(
        self,
        url,
        prefix,
    ):
        url_components = urlsplit(
            url
        )
...
def inject_api_version_header_if_needed(
    ...
    **kwargs,
    ...
    if (
        not model.is_endpoint_discovery_operation
    ):
        return
    params[
        "headers"
        ...
        model.service_model.api_version
    )
...
def add_retry_headers(
    request,
    **kwargs,
):
    retries_context = request.context.get(
        "retries"
    )
    if (
        not retries_context
...
def remove_arn_from_signing_path(
    request,
    **kwargs,
):
    auth_path = (
...
def customize_endpoint_resolver_builtins(
    ...
    **kwargs,
    ...
    path_style_required = (
        bucket_name
        is not None
        and not VALID_HOST_LABEL_RE.match(
            bucket_name
        )
    )
    ...
    if (
        ...
        builtins[
            EndpointResolverBuiltins.AWS_REGION
        ] = "aws-global"
        builtins[
            EndpointResolverBuiltins.AWS_S3_USE_GLOBAL_ENDPOINT
        ] = True


def remove_content_type_header_for_presigning(
    request,
    **kwargs,
):
    if (
...
BUILTIN_HANDLERS = [
    ...
    (
        "creating-client-class.iot-data",
        check_openssl_supports_tls_version_1_2,
    ),
    (
        "creating-client-class.lex-runtime-v2",
    ...
    (
        "after-call.cloudformation.GetTemplate",
    ...
    (
        "docs.*.s3.CopyObject.complete-section",
        document_copy_source_form,
    ),
    (
        "docs.*.s3.UploadPartCopy.complete-section",
        document_copy_source_form,
    ),
    (
    ...
    (
        "before-call.glacier.UploadArchive",
        add_glacier_checksums,
    ),
    (
        "before-call.glacier.UploadMultipartPart",
        add_glacier_checksums,
    ),
    (
        "before-call.ec2.CopySnapshot",
    ...
    (
        "request-created.machinelearning.Predict",
        switch_host_machinelearning,
    ),
    (
        "needs-retry.s3.UploadPartCopy",
        check_for_200_error,
        REGISTER_FIRST,
    ),
    (
    ...
    (
        "choose-signer.cognito-identity.GetId",
        disable_signing,
    ),
    (
        "choose-signer.cognito-identity.GetOpenIdToken",
        disable_signing,
    ),
    (
        "choose-signer.cognito-identity.UnlinkIdentity",
        disable_signing,
    ),
    (
    ...
    (
        "choose-signer.sts.AssumeRoleWithSAML",
        disable_signing,
    ),
    (
        "choose-signer.sts.AssumeRoleWithWebIdentity",
        disable_signing,
    ),
    (
        "choose-signer",
    ...
    (
        "before-parameter-build.s3.HeadObject",
        sse_md5,
    ),
    (
    ...
    (
        "before-parameter-build.s3.SelectObjectContent",
        sse_md5,
    ),
    (
    ...
    (
        "before-parameter-build.route53",
        fix_route53_ids,
    ),
    (
        "before-parameter-build.glacier",
        inject_account_id,
    ),
    (
        "before-sign.s3",
    ...
    (
        "after-call.s3.ListObjectVersions",
