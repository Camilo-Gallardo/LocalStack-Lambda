...
class TokenDecoder:
    ...
    def decode(
        ...
        # be needed.
        encoded_keys = decoded_token.pop(
            "boto_encoded_keys",
            None,
        )
        if (
...
class PaginatorModel:
    def __init__(
        ...
        self._paginator_config = paginator_config[
            "pagination"
        ]
    ...
    def get_paginator(
        ...
        try:
            single_paginator_config = self._paginator_config[
                operation_name
            ]
        except KeyError:
            raise ValueError(
                "Paginator for operation does not exist: %s"
                % operation_name
            )
...
class PageIterator:
    ...
    def resume_token(
        ...
        if (
            ...
            self._resume_token = self._token_encoder.encode(
                value
            )
        else:
            raise ValueError(
    ...
    def __iter__(
        ...
        if (
            ...
            # first response.
            next_token = self._parse_starting_token()[
                0
            ]
        # The number of items from result_key we've seen so far.
        total_items = 0
        first_request = True
        primary_result_key = self.result_keys[
            0
        ]
        starting_truncation = 0
        self._inject_starting_params(
            current_kwargs
        )
        while True:
            ...
            parsed = self._extract_parsed_response(
                response
            )
            if first_request:
                ...
                first_request = False
                self._record_non_aggregate_key_values(
                    parsed
                )
            else:
                ...
                starting_truncation = 0
            current_response = primary_result_key.search(
                parsed
            )
            if (
                current_response
    ...
    def _record_non_aggregate_key_values(
        self,
        response,
    ):
        non_aggregate_keys = (
        ...
        for expression in (
            self._non_aggregate_key_exprs
        ):
            result = expression.search(
                response
            )
            set_value_from_jmespath(
                non_aggregate_keys,
    ...
    def _handle_first_request(
        ...
        starting_truncation = self._parse_starting_token()[
            1
        ]
        all_data = primary_result_key.search(
            parsed
        )
        if isinstance(
            all_data,
    ...
    def _truncate_response(
        ...
        original = primary_result_key.search(
            parsed
        )
        if (
            original
    ...
    def _get_next_token(
        ...
        if (
            ...
            if not self._more_results.search(
                parsed
            ):
                return (
                    {}
        ...
        for (
            ...
            next_token = output_token.search(
                parsed
            )
    ...
    def build_full_result(
        ...
        for response in self:
            ...
            for result_expression in (
                ...
                # as the new value.
                result_value = result_expression.search(
                    page
                )
                if (
                    result_value
                    ...
                    continue
                existing_value = result_expression.search(
                    complete_result
                )
                if (
                    existing_value
    ...
    def _parse_starting_token(
        ...
        try:
            next_token = self._token_decoder.decode(
                next_token
            )
            index = 0
            if (
            ...
            (
                ...
                self._parse_starting_token_deprecated()
            )
        return (
            next_token,
            index,
    ...
    def _parse_starting_token_deprecated(
        self,
    ):
        """
        ...
        parts = self._starting_token.split(
            "___"
        )
        next_token = (
            []
        ...
        return (
            self._convert_deprecated_starting_token(
                next_token
            ),
            index,
        )

    def _convert_deprecated_starting_token(
        self,
        deprecated_token,
    ):
        """
...
class Paginator:
    ...
    def __init__(
        ...
        self._output_token = self._get_output_tokens(
            self._pagination_cfg
        )
        self._input_token = self._get_input_tokens(
            self._pagination_cfg
        )
        self._more_results = self._get_more_results_token(
            self._pagination_cfg
        )
        self._non_aggregate_keys = self._get_non_aggregate_keys(
            self._pagination_cfg
        )
        self._result_keys = self._get_result_keys(
            self._pagination_cfg
        )
        self._limit_key = self._get_limit_key(
            self._pagination_cfg
        )
    ...
    def _get_input_tokens(
        ...
        input_token = self._pagination_cfg[
            "input_token"
        ]
        if not isinstance(
            input_token,
    ...
    def _get_more_results_token(
        self,
        config,
    ):
        more_results = config.get(
            "more_results"
        )
        if (
            more_results
    ...
    def _get_result_keys(
        self,
        config,
    ):
        result_key = config.get(
            "result_key"
        )
        if (
            result_key
    ...
    def paginate(
        ...
        **kwargs,
        ...
        """
        page_params = self._extract_paging_params(
            kwargs
        )
    ...
    def _extract_paging_params(
        self,
        kwargs,
    ):
        pagination_config = kwargs.pop(
            "PaginationConfig",
            {},
        )
        max_items = pagination_config.get(
            "MaxItems",
            None,
        )
        if (
        ...
        page_size = pagination_config.get(
            "PageSize",
            None,
        )
        if (
            ...
            input_members = (
                self._model.input_shape.members
            )
            limit_key_shape = input_members.get(
                self._limit_key
            )
            if (
                limit_key_shape.type_name
                == "string"
            ):
                if not isinstance(
                    page_size,
        ...
        return {
            "MaxItems": max_items,
            "StartingToken": pagination_config.get(
                "StartingToken",
                None,
            ),
            "PageSize": page_size,
...
class ResultKeyIterator:
    ...
    def __iter__(
        ...
        for page in (
            self._pages_iterator
        ):
            results = self.result_key.search(
                page
            )
            if (
                results
