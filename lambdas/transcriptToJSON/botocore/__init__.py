...
def xform_name(
    ...
    if (
        ...
        if (
            _special_case_transform.search(
                name
            )
            is not None
        ):
            is_special = _special_case_transform.search(
                name
            )
            matched = (
                is_special.group()
        ...
        s1 = _first_cap_regex.sub(
            r"\1"
            + sep
            + r"\2",
            name,
        )
        transformed = _end_cap_regex.sub(
            r"\1"
            + sep
            + r"\2",
            s1,
...
def register_initializer(
    ...
    """
    _INITIALIZERS.append(
        callback
    )


def unregister_initializer(
    callback,
):
    ...
    """
    _INITIALIZERS.remove(
        callback
    )


def invoke_initializers(
    session,
):
