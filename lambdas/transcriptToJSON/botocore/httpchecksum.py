...
class AwsChunkedWrapper:
    ...
    def _make_chunk(
        ...
        if (
            ...
            name = self._checksum_name.encode(
                "ascii"
            )
            checksum = self._checksum.b64digest().encode(
                "ascii"
            )
            return (
                b"0\r\n%s:%s\r\n\r\n"
...
class StreamingChecksumBody(
    ...
    def _validate_checksum(
        ...
        if self._checksum.digest() != base64.b64decode(
            self._expected
        ):
            error_msg = (
...
def resolve_request_checksum_algorithm(
    ...
    algorithm_member = http_checksum.get(
        "requestAlgorithmMember"
    )
    if (
        algorithm_member
        and algorithm_member
        in params
    ):
        ...
        if (
            ...
            raise FlexibleChecksumError(
                error_msg="Unsupported checksum algorithm: %s"
                % algorithm_name
            )
...
def apply_request_checksum(
    ...
    if (
        ...
        raise FlexibleChecksumError(
            error_msg="Unknown checksum variant: %s"
            % algorithm[
                "in"
            ]
        )
...
def _apply_request_header_checksum(
    ...
    if (
        ...
        # If the header is already set by the customer, skip calculation
        return
    checksum_cls = _CHECKSUM_CLS.get(
        algorithm[
            "algorithm"
        ]
    )
    digest = checksum_cls().handle(
        request[
            "body"
        ]
    )
...
def _apply_request_trailer_checksum(
    ...
    content_length = determine_content_length(
        body
    )
    if (
        content_length
...
def handle_checksum_body(
    ...
    algorithms = checksum_context.get(
        "response_algorithms"
    )
...
def _handle_bytes_response(
    ...
    if checksum.digest() != base64.b64decode(
        expected
    ):
        error_msg = (
...
if HAS_CRT:
    ...
    assert all(
        name
        in _CRT_CHECKSUM_ALGORITHMS
        for name in _CRT_CHECKSUM_CLS.keys()
    )
_SUPPORTED_CHECKSUM_ALGORITHMS = list(
    _CHECKSUM_CLS.keys()
)
