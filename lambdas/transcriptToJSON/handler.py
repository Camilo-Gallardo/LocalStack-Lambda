...
import logging
import docx2txt
from prompt_to_json import PROMPT_TO_JSON
...
def get_config(event: dict) -> dict:
    ...
    return {
        "bucket": bucket,
        "key": key,
        "output_key": key.replace("transcript/", "json/").replace(".docx", ".json"),
        "index": os.environ.get("INDEX", "sprintdemos"),
    }


def get_s3_object_body(bucket: str, key: str) -> bytes:
...
def invoke_bedrock_model(text: str, video_title: str, index: str) -> dict:
    """Invokes the Bedrock model to convert text to JSON."""
    formatted_prompt = PROMPT_TO_JSON.format(
        index=index, text=text, video_title=video_title
    )

    request_body = {
        "messages": [{"role": "user", "content": [{"text": formatted_prompt}]}],
        "inferenceConfig": {"max_new_tokens": 4000, "temperature": 0.1, "top_p": 0.9},
    }
...
def handler(event, context):
    ...
    try:
        ...
        key, bucket = config["key"], config["bucket"]

        logger.info(f"Processing s3://{bucket}/{key}")

        file_content = get_s3_object_body(bucket, key)
        transcript_text = extract_text_from_docx(file_content)

        video_title = os.path.splitext(os.path.basename(key))[0]
        json_content = invoke_bedrock_model(
            transcript_text, video_title, config["index"]
        )

        save_json_to_s3(bucket, config["output_key"], json_content)

        logger.info(f"Successfully processed and created {config['output_key']}")
        return {
            "statusCode": 200,
        ...
        if config:
            ...
            save_json_to_s3(config["bucket"], config["output_key"], error_response)
