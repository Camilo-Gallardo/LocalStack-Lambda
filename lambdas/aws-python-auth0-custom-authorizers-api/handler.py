...
def jwt_verify(auth_token, public_key):
    ...
    payload = jwt.decode(
        auth_token, pub_key, algorithms=["RS256"], audience=AUTH0_CLIENT_ID
    )
    return payload["sub"]
...
def create_aws_lambda_response(status_code, message, headers):
    return {"statusCode": status_code, "headers": headers, "body": json.dumps(message)}
...
def handler(event, context):
    ...
    if event and event.get("type") == "TOKEN":
        ...
        # principal_id dummy para las pruebas
        return generate_policy("test-user", "Allow", method_arn)
    # fallback para pruebas sencillas
    return create_200_response("ok")
